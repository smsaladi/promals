...
//        then read blastpgp output br file, output it to output_file_name(has a
//        random number) then read output_file_name to a subalign, get rid of
//        positions with gaps in the first sequence
//             the missing ends of blastpgp alignment are filled
//        then purge the subalign by deleting divergent, highly similar, gappy
//        sequences then save the final alignment to a file: query_name.aln then
//        return the final alignment

string tab = "\t";

subalign *get_blastpgp_alignment(char *basename, char *query_name,
                                 char *query_seq) {
  int i, j, k;

  subalign *x;

  // cout << blast_dir << tab << string(blast_dir) << "\t" << string(basename)
  // << tab << endl;
  k = strlen(blast_dir);
  if (blast_dir[k - 1] != '/') {
    assert(k < 500 - 1);
  ...
  // cout << blast_dir << tab << string(blast_dir) << "\t" << string(basename)
  // << tab << endl; cout << ( string(blast_dir) +
  // string(basename)+string(".aln")).c_str() << endl; exit(0);
  x = read_blastpgp_alignment(
      (string(blast_dir) + string(basename) + string(".aln")).c_str(),
  ...
  char optionStr[2000];
  sprintf(optionStr, " -j %d -e %e -h %e -v 1000 -b 1000 -a 2 -m 6 -F %s ",
...
// clean blastpgp directory for files/directories beginning with a certain
// sequene name
...
void set_blastpgp_options(char *options) {
  ...
  if (strlen(options) == 0) {
    strcpy(default_options,
           " -j 3 -e 0.001 -h 0.001 -v 1000 -b 1000 -a 2 -m 6 -F T ");
...
subalign *run_blastpgp(char *query_name, char *query_seq) {
  ...
  fputs(">", fp1);
  fputs(query_name, fp1);
  fputs("\n", fp1);
  for (i = 0; i < strlen(query_seq); i++) {
    if ((query_seq[i] == 'O') || (query_seq[i] == 'o') ||
        (query_seq[i] == 'J') || (query_seq[i] == 'j')) {
      fputs("A", fp1);
      query_OJoj[i] = 'A';
    } else {
  ...
  // run blastpgp
  char command[2000];
  // sprintf(command, "%s -i %s -o %s.br %s -d %s -C %s%s.chk -M %s",
  // blastpgp_cmd, fasta_file_name, fasta_file_name, blastpgp_options,
  // uniref90_file, blast_dir, query_name, blosum62_file);
  sprintf(command, "%s -i %s -o %s.br %s -d %s -C %s%s.chk", blastpgp_cmd,
          fasta_file_name, fasta_file_name, blastpgp_options, uniref90_file,
          blast_dir, query_name);
  ...
  // purge alignment, removing divergent sequences, higly similar sequences,
  // gappy sequences and keep the maximum sequence number to be 50
  ...
  // cout << "low_id_thr: " << low_id_thr << " max_num_sequences: " <<
  // max_num_sequences << endl;

  // output x in the blast_dir directory
  ifstream fp2(
      (string(blast_dir) + string(query_name) + string(".aln")).c_str(),
      ios::in);
  if (!fp2.good()) {
    x->printali(
        (string(blast_dir) + string(query_name) + string(".aln")).c_str(), 60);
  } else {
    fp2.close();
  ...
  // system( (string("rm -f ") + string(br_file_name) +
  // string(".aln")).c_str());
  // system( (string("rm -f ") + string(fasta_file_name) +
  // string(".aln")).c_str());
  // system( (string("mv ") + string(fasta_file_name) + string(" ") +
  // string(fasta_file_name1) ).c_str() );
...
/*
subalign *get_blastpgp_result(const char *blastpgp_result, char *query_name,
char *query_seq) {

        subalign *x = read_blastpgp_result(blastpgp_result, query_name,
query_seq);
...
// new: add option of reading low complexity region in the query if the query is
// filtered
subalign *read_blastpgp_result(const char *blastpgp_result, char *query_name,
                               char *query_seq) {
  int i, j, k;
  int round = 0;
  ...
  if ((fp = fopen(blastpgp_result, "r")) == NULL) {
    ...
    // fprintf(stderr, "and output the alignment from the last round of
  ...
  if (!fp1) {
    cout << "cannot open the temporary alignment file for write "
         << output_aln_file << endl;
    exit(0);
  }
  ...
  while (fgets(str, 200, fp) != NULL) {
    ...
    if (strncmp(str, " ***** No hits found ******", 27) == 0) {
      flag = 0;
    }
  ...
  if (flag1 == 0) {
    cout << "nothing is read from blast output file " << output_aln_file
         << endl;
  ...
  for (i = 0; i < strlen(tmpstring); i++) {
    ...
    if ((tmpstring[i] != query_seq[i + right_numbers]) &&
        (tmpstring[i] != query_seq[i + right_numbers] - 32)) {
  ...
  // purge_alignment_by_first_sequence(x); return x;  }
  ...
  for (i = 0; i < x->nal; i++) {
    if (i == 0) {
      ...
      // replace the 'X' characters (filtered segments) according to original
      // query sequence
      k = 0;
      ...
      for (j = 0; j < strlen(tmpseq[i]); j++) {
        if (tmpseq[i][j] != '-') {
          k++;
          query_filter[k - 1] =
              tmpseq[i][j];  // record the filtered query sequence
        }
      ...
      for (j = 0; j < strlen(x->aseq[0]); j++) {
        ...
        if (tmpseq[i][j + strlen(Nterm_gap.c_str())] == 'X')
          tmpseq[i][j + strlen(Nterm_gap.c_str())] = '-';
      }
    }
  }
  ...
  strcpy(query_filter_file,
         (string(blastpgp_result) + string(".filter.fa")).c_str());
...
subalign *read_blastpgp_alignment(const char *blastpgp_alignment,
