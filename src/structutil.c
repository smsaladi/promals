...
subalign *merge_align_by_one_sequence_insert(subalign *a, subalign *b,
  ...
  // NEW: modify the sequences if small letters are found, replace them with gap
  // characters
  /*
  ...
  */
  ...
  for (i = 0; i < a->nal; i++) {
    if (i <= ia)
      ...
      strcpy(new_aln->aname[i + b->nal - 1], a->aname[i]);
  }
  for (i = 0; i < b->nal; i++) {
    if (i == ib) continue;
  ...
  for (i = 0; i < a->nal; i++) {
        ...
        for (l = 1; l <= gap_pattern_b[tmp_index]; l++) {
          for (k = 0; k < a->nal; k++) {
            if (k <= ia)
              ...
              new_aln->aseq[k + b->nal - 1][atmp_len] = '-';
          }
          atmp_len++;
        }
        for (k = 0; k < a->nal; k++) {
          if (k <= ia)
            ...
            new_aln->aseq[k + b->nal - 1][atmp_len] = a->aseq[k][j];
        }
        atmp_len++;
        tmp_index += 1;
      } else {
    ...
    for (k = 0; k < a->nal; k++) {
      if (k <= ia)
        ...
        new_aln->aseq[k + b->nal - 1][atmp_len] = '\0';
    }
  }
