...
void get_added_gap_number_1(char *seq1, char *seq2, int *added_gap_number_1, int dim);
...
void combine_structure_alignments(btree<tnode> &alltree, vector<seq_str_aln *> &ssaln) {
		...
		for(j=i+1;j<(int)alltree.preAligned.size();j++) {
			...
			for(k=1;k<=alltree.preAligned[i]->aln->alilen;k++) {
				for(l=1;l<=alltree.preAligned[j]->aln->alilen;l++) {
					...
					tmpMat[k][l] = tmpMat[k][l]*sequence_weight + structmat[k][l]*struct_weight;
				}
			}
                        //cout << "This place" << endl;
			delete smat[i][j];
			delete smat[j][i];
			smat[i][j] = new sparseMatrix(tmpMat,alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			smat[j][i] = smat[i][j]->transpose();
			free_gmatrix<float>(tmpMat, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
                        //cout << "This place" << endl;
		}
	}
}
...
void combine_structure_alignments3(btree<tnode> &alltree, vector<seq_str_aln *> &ssaln) {
	...
	float **structmat_dali=NULL, **structmat_fast=NULL, **structmat_tmalign=NULL, **tmpMat, **structmat;
	sparseMatrix ***smat = alltree.smat;
	...
	for(i=0;i<(int)alltree.preAligned.size();i++) {
		for(j=i+1;j<(int)alltree.preAligned.size();j++) {
			//cout << "+++++++++++" << i << " " << j << endl;
                      ...
                      if(realign_psiblast) {
                        if(use_dali) structmat_dali = map_structure_promals(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "dali");
                        if(use_fast) structmat_fast = map_structure_promals(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "fast");
                        cout << "Here fast: " << endl;
                        if(use_tmalign) structmat_tmalign = map_structure_promals(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "tmalign");
                      }
                      else {
                        if(use_dali) structmat_dali = map_structure_nopromals(ssaln[i], ssaln[j], "dali");
                        if(use_fast) structmat_fast = map_structure_nopromals(ssaln[i], ssaln[j], "fast");
                        if(use_tmalign) structmat_tmalign = map_structure_nopromals(ssaln[i], ssaln[j], "tmalign");
                      }
			if( (!structmat_dali) && (!structmat_fast) && (!structmat_tmalign) ) continue;
                        structmat = gmatrix<float>(alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
                        for(k=1;k<=alltree.preAligned[i]->aln->alilen;k++) {
                                for(l=1;l<=alltree.preAligned[j]->aln->alilen;l++) {
                                        structmat[k][l] = 0;
                                }
                        }
                        if(use_dali) combine_structmat(structmat, structmat_dali, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen, 1.0);
                        if(use_fast) combine_structmat(structmat, structmat_fast, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen, 1.0);
                        //cout << "Here fast2: " << endl;
                        if(use_tmalign) combine_structmat(structmat, structmat_tmalign, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen, 1.0);
                        // debugging
                        /*cout << alltree.preAligned[i]->aln->aname[0] << " " << alltree.preAligned[j]->aln->aname[0] << endl;
			for(k=1;k<=alltree.preAligned[i]->aln->alilen;k++) {
				for(l=1;l<=alltree.preAligned[j]->aln->alilen;l++) {
                                        if(structmat_dali[k][l]) {
                                                cout << k << " " << l << " " << alltree.preAligned[i]->aln->aseq[0][k-1] << " " << alltree.preAligned[j]->aln->aseq[0][l-1] << " " << structmat_dali[k][l] << endl;
                                        }
				}
			}*/
			...
			free_gmatrix<float>(tmpMat, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat_dali, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat_fast, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat_tmalign, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
                        //cout << "This place" << endl;
		}
	}
}
...
void combine_structure_alignments3_updated(btree<tnode> &alltree, vector<seq_str_aln *> &ssaln) {
	...
	for(i=0;i<(int)alltree.preAligned.size();i++) {
		for(j=i+1;j<(int)alltree.preAligned.size();j++) {
			//cout << "+++++++++++" << i << " " << j << endl;
                        ...
                        structmat_tmalign = NULL;
                      if(realign_psiblast) {
                        structmat_dali = map_structure_promals_updated(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], use_dali, use_fast, use_tmalign);
                        //if(use_dali) structmat_dali = map_structure(ssaln[i], ssaln[j]);
                        /*
                        if(use_dali) structmat_dali = map_structure_promals_updated(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "dali");
			//if(use_fast) structmat_fast = map_structure_fast(ssaln[i], ssaln[j]);
                        if(use_fast) structmat_fast = map_structure_promals_updated(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "fast");
                        //cout << "Here fast updated: " << endl;
			//if(use_tmalign) structmat_tmalign = map_structure_tmalign(ssaln[i], ssaln[j]);
                        if(use_tmalign) structmat_tmalign = map_structure_promals_updated(alltree.preAligned[i]->aux_align, alltree.preAligned[j]->aux_align, ssaln[i], ssaln[j], "tmalign");
                        */
                      }
                      else {
                        if(use_dali) structmat_dali = map_structure_nopromals_updated(ssaln[i], ssaln[j], "dali");
                        if(use_fast) structmat_fast = map_structure_nopromals_updated(ssaln[i], ssaln[j], "fast");
                        if(use_tmalign) structmat_tmalign = map_structure_nopromals_updated(ssaln[i], ssaln[j], "tmalign");
                      }
			if( (!structmat_dali) && (!structmat_fast) && (!structmat_tmalign) ) continue;
                        structmat = gmatrix<float>(alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
                        for(k=1;k<=alltree.preAligned[i]->aln->alilen;k++) {
...
//
void combine_structmat(float **mat1, float **mat2, int d1, int d2, float weight) {
        int i, j;
        if(!mat2) return;
        ...
        for(i=1;i<=d1;i++) {
                for(j=1;j<=d2;j++) {
                        if(mat2[i][j]) {
                                if(mat1[i][j]) {mat1[i][j] += weight;}
                                else {mat1[i][j] = mat2[i][j];}
                        }
                }
        }
        return;
...
// combine structure alignment for dali, fast and tmalign
void combine_constraint(btree<tnode> &alltree, constraint &cons, float constraint_w) {
	...
	for(i=0;i<(int)alltree.preAligned.size();i++) {
                ...
                if(ci==-1) continue; // if not found, there is no constraint on prealigned group i
		for(j=i+1;j<(int)alltree.preAligned.size();j++) {
                        ...
                        if(cj==-1) continue; // if not found, there is no constraint on prealigned group i
			...
			for(k=1;k<=alltree.preAligned[i]->aln->alilen;k++) {
				for(l=1;l<=alltree.preAligned[j]->aln->alilen;l++) {
					//tmpMat[k][l] += structmat[k][l]*struct_weight;
					tmpMat[k][l] = tmpMat[k][l] + structmat_constraint[k][l]*constraint_w;
                                        cout << k << " " << l <<  " " << alltree.preAligned[i]->aln->aseq[0][k-1] << " " << alltree.preAligned[j]->aln->aseq[0][l-1]<< " " << tmpMat[k][l] << endl;
				}
			}
                        //cout << "This place" << endl;
			delete smat[i][j];
			...
			smat[j][i] = smat[i][j]->transpose();
			free_gmatrix<float>(tmpMat, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
			free_gmatrix<float>(structmat_constraint, alltree.preAligned[i]->aln->alilen, alltree.preAligned[j]->aln->alilen);
                        //cout << "This place" << endl;
		}
	}
}
...
int **get_salnpos(char *name1, char *name2, int &numalignedp) {
	...
	//cout << "names: " << name1 << " " << name2 << " " << address_daliscore[0] << " " << address_daliscore[1] << endl;
	fclose(idxfp);
	...
	for(i=0;i<strlen(alnseq1);i++) {
		...
		if( (alnseq1[i]>=65) && (alnseq1[i]<=90) ) {
			...
			//cout << countp << " " <<  salpos[0][countp] << " " << salpos[1][countp] << endl;
			countp++;
		}
	}
...
// get aligned residue positions for two dali id names
int **get_salnpos_updated(seq_str_aln *a1, seq_str_aln *a2, int ind1, int ind2, int &numalignedp, const char *prog_name) {
	...
	strcpy(index_dir, "/home/jpei/db/tmalign_1.69/");
     ...
     if(debug_here > 11) {
        ...
        // check if the pdb code ids are the same; if the same, no need to run structural comparison program
        if(strcmp(a1->id[ind1], a2->id[ind2])==0) {
	        ...
	        salpos = imatrix(1, numalignedp);
                ...
                if(debug_here>11)cout << "maxstart: " << maxstart << " minend: " << minend << endl;
                for(i=maxstart;i<=minend;i++) {
                        salpos[0][i-maxstart] = i;
                        salpos[1][i-maxstart] = i;
                        // Debug
                        if(debug_here>11)cout << salpos[0][i-maxstart] << " " << salpos[1][i-maxstart] << endl;
                }
                return salpos;
        }
                        
        // pdb code ids are not the same, run structural comparison programs by calling a python wrapper
        char command[500];
        char tmpstr[10000];
        char *ss;
        int start_recording = 0;
        //sprintf(command, "/home/jpei/evolvable_database/bin/get_struct_constraint.py %s %s -%s -dirname %s -start1 %d -start2 %d -end1 %d -end2 %d -clear 1", a1->id[ind1], a2->id[ind2], prog_name, blast_dir, a1->str_start[ind1], a2->str_start[ind2], a1->str_end[ind1], a2->str_end[ind2]);
        sprintf(command, "python %s/prog/saln/get_struct_constraint.py %s %s -%s -dirname %s -start1 %d -start2 %d -end1 %d -end2 %d -clear 1 -pdbdir %s/db/structure_db -progdir %s/bin", program_dir, a1->id[ind1], a2->id[ind2], prog_name, blast_dir, a1->str_start[ind1], a2->str_start[ind2], a1->str_end[ind1], a2->str_end[ind2], program_dir, program_dir);
        cout << "Command: " << endl;
        cout << command << endl;
        ...
        while(fgets(tmpstr, 10000, pfp)) {
                ...
                // Debug
                //cout << salpos[0][start_recording-1] << " " << salpos[1][start_recording-1] << endl;
                start_recording++;
        }
        pclose(pfp);
        cout << endl;
...
float **map_structure_nopromals(seq_str_aln *a1, seq_str_aln *a2, const char *prog_name) {
...
float **map_structure_promals(subalign *auxa, subalign *auxb, seq_str_aln *a1, seq_str_aln *a2, const char *prog_name) {

        int i, j, k, l;

	int **salnpos=NULL; // structure alignment positions; two arrays of integer values
	// [0..1][0..numalignedp-1]
	// Example:
	// aCKE-EKaa
 	// -CEEKEKa-
	...
	for(i=1;i<=a1->nhits;i++) {
		for(j=1;j<=a2->nhits;j++) {
			        ...
			        salnpos = get_salnpos_tmalign(a1->id[i], a2->id[j], numalignedp);
                        ...
                        //cout << "here1" << endl;
                        for(k=1;k<=a1->slen[i];k++) for(l=1;l<=a2->slen[j];l++) m_bc[k][l] = 0;
                        //cout << "here" << endl;
                        //cout << a1->query << " " << a1->id[i] << endl;
                        //cout << a2->query << " " << a2->id[j] << endl;
                        for(k=0;k<numalignedp;k++) {
                                //cout << a1->slen[i] << " " << a2->slen[j] << " " << salnpos[0][k] << " " << salnpos[1][k] << endl;
                        ...
                        // 3. get the residue match probability matrix between target a and template b
		        ...
		        for(k=1;k<=lenx;k++){for(l=1;l<=leny;l++){if(realmat[k][l]<minProb)realmat[k][l] = 0;}}
			sparseMatrix *m_ab_S = new sparseMatrix(realmat, lenx, leny);
			delete hmmProfPair;
                        //cout << "here" << endl;

                        // 4. get the residue match probability matrix between template c and target d
			...
			delete hmmProfPair;
                        ...
                        for(k=1;k<=lenx;k++) for(l=1;l<=leny;l++) realmat[k][l] = 0;
                        relaxTwoSparse(m_ab_S, m_bc_S, realmat);
                        //cout << "here___" << endl;
                        sparseMatrix *m_abc_S = new sparseMatrix(realmat, lenx, a2->prof[j]->alilen);
                        //cout << "here___" << endl;
                        ...
                        for(k=1;k<=lenx;k++) for(l=1;l<=leny;l++) tmpMat[k][l] += realmat[k][l];
                        free_gmatrix(realmat, lenx, leny);
                        //cout << "here___=======" << endl;
        ...
        for(i=1;i<=a1->len;i++) {
                for(j=1;j<=a2->len;j++) {
                        if (tmpMat[i][j] < minProb) tmpMat[i][j]=0;
                                ...
                                //cout << "tmpMat: " << i << " " << j << " " << auxa->aseq[0][i-1] << " " << auxb->aseq[0][j-1]<< tmpMat[i][j] << endl;
                                tmpMat_count++;
                        }
                }
        }
...
float **map_structure_nopromals_updated(seq_str_aln *a1, seq_str_aln *a2, const char *prog_name) {
	...
	for(i=1;i<=a1->nhits;i++) {
		for(j=1;j<=a2->nhits;j++) {
			...
			for(k=0;k<numalignedp;k++) {
				if(debug_here>11)cout << "1: " << salnpos[0][k] <<  " " << a1->aln[i][salnpos[0][k]] << endl;
				p1 = a1->aln[i][salnpos[0][k]];
				if(!p1) {
                                        //cout << "not p1" << endl;
                                        continue;
                                }
                                //cout << a2->id[j] << " " <<  a2->slen[j] << " " <<  a2->aln[j][a2->aln[j][a2->slen[j]-1]] << endl;
                                //for(l=1;l<=a2->slen[j];l++) { cout << "l: " << l << " " << a2->aln[j][l] << endl; }
				//cout << "2: " << endl; cout << salnpos[1][k] <<  endl; cout <<  " " << a2->aln[j][salnpos[1][k]] << endl;
				p2 = a2->aln[j][salnpos[1][k]];
				if(!p2) continue;
...
// prog_name: the name of the structural alignment program
float **map_structure_promals_updated(subalign *auxa, subalign *auxb, seq_str_aln *a1, seq_str_aln *a2, int dali, int fast, int tmalign) {
	...
	for(i=1;i<=a1->nhits;i++) {
		for(j=1;j<=a2->nhits;j++) {
                        ...
                        // 1. get the structural alignment positions
                        //cout << "here" << endl;
                        if(dali) salnpos = get_salnpos_updated(a1, a2, i, j, numalignedp, "dali");
                        if(fast) salnpos1 = get_salnpos_updated(a1, a2, i, j, numalignedp1, "fast");
                        if(tmalign) salnpos2 = get_salnpos_updated(a1, a2, i, j, numalignedp2, "tmalign");
                        ...
                        //cout << "here1" << endl;
                        for(k=1;k<=lenx;k++) for(l=1;l<=leny;l++) m_bc[k][l] = 0;
                        //cout << "here" << endl;
                        //cout << a1->query << " " << a1->id[i] << endl;
                        //cout << a2->query << " " << a2->id[j] << endl;
                        //cout << "numalignedp: " << numalignedp << endl;
                     if(salnpos) {
                        for(k=0;k<numalignedp;k++) {
                                //cout << k << " " << a1->slen[i] << " " << a2->slen[j] << " " << salnpos[0][k] << " " << salnpos[1][k] << endl;
                     ...
                     if(salnpos1) {
                        for(k=0;k<numalignedp1;k++) {
                                //cout << k << " " << a1->slen[i] << " " << a2->slen[j] << " " << salnpos[0][k] << " " << salnpos[1][k] << endl;
                                cout << k << " " << salnpos1[0][k]-offset_b << " " << salnpos1[1][k]-offset_c << endl;
                     ...
                     if(salnpos2) {
                        for(k=0;k<numalignedp2;k++) {
                                //cout << k << " " << a1->slen[i] << " " << a2->slen[j] << " " << salnpos[0][k] << " " << salnpos[1][k] << endl;
                                cout << k << " " << salnpos2[0][k]-offset_b << " " << salnpos2[1][k]-offset_c << endl;
...
// prog_name: the name of the structural alignment program
/*
float **map_structure_promals_updated(subalign *auxa, subalign *auxb, seq_str_aln *a1, seq_str_aln *a2, char *prog_name) {
...
// merge a and b according to the sequence with seq_name
// b is added to a
subalign * merge_align_by_one_sequence_insert(subalign *a, subalign *b, char *seq_name) {
	...
	if(ib<0) { cout << "Name " << seq_name << " is not present in b" << endl; exit(0); }
        ...
        // NEW: modify the sequences if small letters are found, replace them with gap characters
        /*
        vector<int> smallletterindex;
        int myindex = -1;
        string tmpstr = "";
        for(i=0;i<a->alilen;i++) {
                if( a->aseq[ia][i]!='-') myindex++;
                if( (a->aseq[ia][i]!='-') && (a->aseq[ia][i]>'Z') ) {
                        tmpstr += a->aseq[ia][i];
                        a->aseq[ia][i] = '-';
                        smallletterindex.push_back(myindex);
                }
        }
        if(tmpstr.size()>0) cout << "sla: " << tmpstr << endl;
     if(smallletterindex.size()>0) {
        myindex = -1;
        for(i=0;i<b->alilen;i++) {
                if(b->aseq[ib][i]!='-') {
                        myindex++;
                        for(j=0;j<smallletterindex.size();j++) {
                                if(smallletterindex[j]==myindex) {
                                        b->aseq[ib][i] = '-';
                                        break;
                                }
                        }
        ...
        smallletterindex.clear();
        myindex = -1;
        tmpstr = "";
        for(i=0;i<b->alilen;i++) {
                if( b->aseq[ib][i]!='-') myindex++;
                if( (b->aseq[ib][i]!='-') && (b->aseq[ib][i]>'Z') ) {
                        tmpstr += b->aseq[ib][i];
                        b->aseq[ib][i] = '-';
                        smallletterindex.push_back(myindex);
                }
        }
        if(tmpstr.size()>0) cout << "slb: " << tmpstr << endl;
     if(smallletterindex.size()>0) {
        myindex = -1;
        for(i=0;i<a->alilen;i++) {
                if(a->aseq[ia][i]!='-') {
	...
	for(i=0;i<a->nal;i++) {
                if(i<=ia) strcpy(new_aln->aname[i], a->aname[i]);
                else strcpy(new_aln->aname[i+b->nal-1], a->aname[i]);
	}
	for(i=0;i<b->nal;i++) {
		if(i==ib) continue;
		if(i<ib) strcpy(new_aln->aname[i+ia+1], b->aname[i]);
		if(i>ib) strcpy(new_aln->aname[i+ia], b->aname[i]);
	}
	...
	for(i=0;i<a->nal;i++) {
		...
		for(j=0;j<a->alilen;j++) {
			if(a->aseq[i][j]!='-') {
				for(l=1;l<=gap_pattern_b[tmp_index];l++) {
					for(k=0;k<a->nal;k++) {
                                                if(k<=ia) new_aln->aseq[k][atmp_len] = '-';
                                                else new_aln->aseq[k+b->nal-1][atmp_len] = '-';
					}
					atmp_len++;
				}
				for(k=0;k<a->nal;k++) {
                                        if(k<=ia) new_aln->aseq[k][atmp_len] = a->aseq[k][j];
                                        else new_aln->aseq[k+b->nal-1][atmp_len] = a->aseq[k][j];
				}
				atmp_len++;
				tmp_index+=1;
			}
		...
		// closing the sequences
		for(k=0;k<a->nal;k++) {
			if(k<=ia) new_aln->aseq[k][atmp_len] = '\0';
                        else  new_aln->aseq[k+b->nal-1][atmp_len] = '\0';
		}
	}
	...
	for(i=0;i<b->nal;i++) {
			...
			if(b->aseq[i][j]!='-') {
				for(l=1;l<=gap_pattern_a[tmp_index];l++) {
					for(k=0;k<b->nal;k++) {
						// skipping the linking sequence
						if(k==ib) continue;
						if(k<ib) new_aln->aseq[k+ia+1][atmp_len] = '-';
						if(k>ib) new_aln->aseq[k+ia][atmp_len] = '-';
					}
					atmp_len++;
				}
				for(k=0;k<b->nal;k++) {
					if(k==ib) continue;
					if(k<ib) new_aln->aseq[k+ia+1][atmp_len] = b->aseq[k][j];
					if(k>ib) new_aln->aseq[k+ia][atmp_len] = b->aseq[k][j];
				}
				atmp_len++;
				tmp_index+=1;
			}
		...
		for(k=0;k<b->nal;k++) {
			if(k==ib) continue;
			if(k<ib) new_aln->aseq[k+ia+1][atmp_len] = '\0';
			if(k>ib) new_aln->aseq[k+ia][atmp_len] = '\0';
		}
	}
...
subalign * merge_master_and_slaves(subalign *a, vector<subalign *> &b, vector<char*> &seq_name) {
        ...
        for(i=0; i<ns; i++) {
                ...
                if(found==0) {
                        cout << "Error: cannot find the representative name: " << seq_name[i] << endl;
                        b[i]->printali(80);
                        exit(0);
                }
        }
        ...
        if(found!=ns){
                cout << "Error: cannot find all the representative names in subalign a" << endl;
                exit(0);
        }
        ...
        for(i=0;i<ns;i++) {
                get_added_gap_number_1(a->aseq[rep_index_in_a[i]], b[i]->aseq[rep_index_in_b[i]], added_gap_number_1, a->alilen);
                for(j=0;j<=lena;j++) {
        ...
        if(debug>1) for(j=0;j<=lena;j++) { cout << "added_gap_number: " << j << " " << added_gap_number[j] << endl; }
        ...
        for(i=0;i<ns;i++) {
                ...
                //cout << b[i]->nal-1 << endl;
                if(newalign->mnamelen<b[i]->mnamelen) newalign->mnamelen = b[i]->mnamelen;
        }
        //cout << "newalign: " << endl;
        ...
        for(i=0;i<a->nal;i++) {
                ...
                //get_added_gap_number_1(b[bi]->aseq[rep_index_in_b[bi]], a->aseq[i], added_gap_number_1, b[bi]->alilen);
                get_added_gap_number_1(b[bi]->aseq[rep_index_in_b[bi]], newseq[repindex], added_gap_number_1, b[bi]->alilen);
                lenb = b[bi]->alilen;
...
void get_added_gap_number_1(char *seq1, char *seq2, int *added_gap_number_1, int dim) {
        ...
        while(i1<=dim) {
                ...
                if(debug>1) cout << *s << " " << *ss << " " << g1 << " " << g2 << " " << i1 << endl;
                i1++;
                if(i1>dim) break;
                s++;
                ss++;
