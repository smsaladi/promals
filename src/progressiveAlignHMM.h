...
void btree<TNODE>::progressiveAlignHMM(TNODE *n) {
  ...
  a->beta = 0;
  b->beta = 0;  // make the pseudocount weak
...
void btree<TNODE>::progressiveAlignHMM_FastStage(TNODE *n, float distCutoff) {
  ...
  if (Debug > 1) {
    if (dist > distCutoff) {
      if (n->childL->aligned) {
        cout << "Just aligned size: " << n->childL->aln->alilen << " "
             << n->childL->aln->nal << " "
             << n->childL->aln->alilen * n->childL->aln->nal << endl;
      }
      if (n->childR->aligned) {
        cout << "Just aligned size: " << n->childR->aln->alilen << " "
             << n->childR->aln->nal << " "
             << n->childR->aln->alilen * n->childR->aln->nal << endl;
      }
    }
  }
  if ((!n->childL->aligned) || (!n->childR->aligned)) {
    return;
  }
  ...
  // NEW debug
  if (Debug > 1) {
    cout << "The alignment size: " << n->aln->alilen << " " << n->aln->nal
         << " " << n->aln->alilen * n->aln->nal << endl;
  }
  if (Debug > 1) n->aln->printali(70);
  if (Debug > 1) cout << "------------" << endl;
  /*
...
// return an array of leaf tnode indexes, which will be used to extract
// sequences and their names
template <typename TNODE>
vector<int> btree<TNODE>::progressiveAlignHMM_FastStage_mafft(
    TNODE *n, float distCutoff) {
  int i, j;
  // subalign *a, *b;
  ...
  // 4. if distance to the leafs is smaller or equal to distCutoff, check the
  // distance of its parent 4.1 a leaf node
  if (n->childL == NULL) {
    vector<int> index;
    // cout << "n-n: " << n->n << endl;
    index.push_back(n->n);
  ...
  float parent_dist;
  if (n->rootFlag)
    parent_dist = dist + 1.0;
  else
    parent_dist = dist + n->branchlen;
  vector<int> index1 =
      progressiveAlignHMM_FastStage_mafft(n->childL, distCutoff);
  vector<int> index2 =
      progressiveAlignHMM_FastStage_mafft(n->childR, distCutoff);
  ...
  // if the distance of its parent is smaller than distCutoff,  just return the
  // vector of indexes also set this node status to be 'aligned'
  if (parent_dist <= distCutoff) {
    n->aligned = 1;
    return index;
  }

  // 5. distance is smaller or equal to distCutoff, while distance of parent is
  // larger than distCutoff
  ...
  sprintf(command, "%s --auto %s/tmp.fa 1>%s/tmp.aln.fa 2>%s/tmp.err", mafft,
...
void btree<TNODE>::obtainPreAligned(TNODE *n) {
  ...
  if(!n->rootFlag) cout << "aligned: " << n->aligned << " parent_aligned: " <<
  n->parent->aligned << endl; if(n->aligned) n->aln->printali(70); tnode *T =
  n->childL; double len = 0; while(T) { len += T->branchlen; T = T->childL;
  }
  cout << "distance to the leaf node: " << len << endl;
  */
...
void btree<TNODE>::getProfileForPreAligned() {
  ...
  for (i = 0; i < preAligned.size(); i++) {
    preAligned[i]->aln->gap_threshold =
        1;  // this means all positions are included regardless of gap content
    preAligned[i]->aln->beta =
        0;  // this means do not mix with pseudocount frequencies; that is, only
            // raw frequencies are used for the profile
...
void btree<TNODE>::profileConsistency() {
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      ...
      if (debug > 1)
        cout << "None-zero counts: " << i << "\t" << j << "\t" << nonZeroCounts
             << endl;
      // cout << "Here " << endl;
  ...
  if (debug > 1) {
    cout << "Time after the initial generation of consistency matrices: "
         << endl;
    times(&tmsend);
    timeDiff();
    times(&tmsstart);
...
void btree<TNODE>::profileConsistency_multim(hmm_parameters *params) {
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      ...
      if (reverse_align_order) {
        hmm_multim *hmmProfPair1 =
            new hmm_multim(preAligned[j]->aln, preAligned[i]->aln);
        hmmProfPair1->set_parameters(params);
        hmmProfPair1->forward();
        hmmProfPair1->backward();
        realmat1 = hmmProfPair1->probMat;
        lenx1 = hmmProfPair1->lenx;
        leny1 = hmmProfPair1->leny;
  ...
  for(i=0;i<(int)preAligned.size();i++) {
          for(j=i+1;j<(int)preAligned.size();j++) {
              cout << "####### " << i << "   " << j << endl;
              cout << preAligned[i]->aln->aname[0] << " " <<
  preAligned[j]->aln->aname[0] << endl; smat[i][j]->printSparseMatrix(1);
          }
  }
  */
}
...
template <typename TNODE>
void btree<TNODE>::profileConsistency_psipred(hmm_psipred_parameters *params,
                                              double **dist_matrix,
                                              double max_dist_cutoff,
                                              int use_homologs) {
  int i, j = 0, k, l, m, n;
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    ...
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      // cout << "i: " << i << "\t" << "j: " << "\t" << j << endl;
      nonZeroCounts = 0;
      hmm_psipred *hmmProfPair =
          new hmm_psipred(preAligned[i]->aux_align, preAligned[j]->aux_align);
      ...
      leny = hmmProfPair->leny;
      if (reverse_align_order) {
        hmm_psipred *hmmProfPair1 =
            new hmm_psipred(preAligned[j]->aln, preAligned[i]->aln);
        hmmProfPair1->set_parameters(params);
      ...
      // cout << "####### " << i << "   " << j << endl;
      // cout << preAligned[i]->aln->aname[0] << " " <<
      // preAligned[j]->aln->aname[0] << endl;
      // smat[i][j]->printCrs(preAligned[i]->aln->aseq[0],
      // preAligned[j]->aln->aseq[0]); smat[i][j]->printSparseMatrix(1); cout <<
      // "Here " << endl;
      if (Debug > 1) smat[i][j]->printCrs();
      // smat[i][j] = new sparseMatrix;
      // smat[i][j]->nrows=0; smat[i][j]->ncols=0;
  ...
  for(i=0;i<relax_number;i++) {
      //cout << " Before relax" << endl;
      relaxConsistMatrix(dist_matrix, max_dist_cutoff);
      fprintf(logfp, "        finished relaxing consistency matrix - round %d of
  %d\n", i+1, relax_number); fflush(logfp);
      //cout << " After relax" << endl;
  }
  */
...
template <typename TNODE>
void btree<TNODE>::profileConsistency_psipred_sum_of_pairs(
    hmm_psipred_parameters *params, double **dist_matrix,
    double max_dist_cutoff, int use_homologs) {
  int i, j = 0, k, l, m, n;
  float **realmat, **realmat1;
  int lenx, leny, lenx1, leny1;
  int nonZeroCounts = 0;
...
// unfinished
template <typename TNODE>
void btree<TNODE>::profileConsistency_profilehmm(hmm_parameters *params,
                                                 char *ss_dir_name, int use_ss,
                                                 float ss_weight,
                                                 double **dist_matrix,
                                                 double max_dist_cutoff) {
  int i, j = 0, k, l, m, n;
...
template <typename TNODE>
void btree<TNODE>::profileConsistency_multim2(hmm_parameters *params1,
                                              hmm_parameters *params2,
                                              sequences *tmpSeq2,
                                              double id_cutoff) {
  int i, j = 0, k, l, m, n;
  float **realmat, **realmat1;
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      ...
      dist =
          tmpSeq2->distMat
              [tmpSeq2->name2index.find(preAligned[i]->aln->aname[0])->second]
              [tmpSeq2->name2index.find(preAligned[j]->aln->aname[0])->second];
      // cout << preAligned[i]->aln->aname[0] << " " <<
      // tmpSeq2->name2index.find(preAligned[i]->aln->aname[0])->second << " " <<
      // preAligned[j]->aln->aname[0] << " " <<
      // tmpSeq2->name2index.find(preAligned[j]->aln->aname[0])->second << " " <<
      // dist << endl;
      hmm_multim *hmmProfPair =
          new hmm_multim(preAligned[i]->aln, preAligned[j]->aln);
      if (dist > 1 - id_cutoff)
        ...
        hmmProfPair->set_parameters(params2);
      ...
      /*
      if(reverse_align_order) {
              hmm_multim *hmmProfPair1 = new hmm_multim(preAligned[j]->aln,
      preAligned[i]->aln); hmmProfPair1->set_parameters(params);
              hmmProfPair1->forward();
              hmmProfPair1->backward();
              realmat1 = hmmProfPair1->probMat;
...
void btree<TNODE>::profileConsistency_local() {
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      nonZeroCounts = 0;
      hmm_local *hmmProfPair =
          new hmm_local(preAligned[i]->aln, preAligned[j]->aln);
      hmmProfPair->forward();
      hmmProfPair->backward();
      realmat = hmmProfPair->probMat;
      lenx = hmmProfPair->lenx;
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      ...
      for (k = 1; k <= lenx; k++) {
        for (l = 1; l <= leny; l++) {
          realmat[k][l] =
              wg * hmmProfPair_g->probMat[k][l] + (1 - wg) * realmat[k][l];
        }
      }
      for (k = 1; k <= lenx; k++) {
        for (l = 1; l <= leny; l++) {
...
void btree<TNODE>::alignment_consistency(vector<sequences> seq_vector) {
  ...
  for (i = 0; i < (int)preAligned.size(); i++) {
    for (j = i + 1; j < (int)preAligned.size(); j++) {
      lenx = preAligned[i]->aln->alilen;
      leny = preAligned[j]->aln->alilen;
      realmat = gmatrix<float>(lenx, leny);
      for (k = 0; k <= lenx; k++) {
        for (l = 0; l <= leny; l++) realmat[k][l] = 0;
      }
...
void btree<TNODE>::relaxConsistMatrix() {
  ...
  if (debug > 1) {
    ...
    for (i = 0; i < preAligned.size(); i++) {  // Update the matrices
      for (j = 0; j < preAligned.size(); j++) {
        if (i == j) {
          cout << "0\t";
          if (i == preAligned.size() - 1) cout << endl;
          continue;
        }
  ...
  for (i = 0; i < preAligned.size(); i++) {
    for (j = i + 1; j < preAligned.size(); j++) {
      ...
      // cout << "HERE: " << endl;
      smat1[i][j] = new sparseMatrix(tmpMat, preAligned[i]->aln->alilen,
                                     preAligned[j]->aln->alilen);
      // cout << "HERE: " << endl;
      // normalize
      // smat1[i][j]->multiplyConstant(1.0/preAligned.size() );
      smat1[j][i] = smat1[i][j]->transpose();
      // smat1[i][j]->printCrs();
      free_gmatrix<float>(tmpMat, preAligned[i]->aln->alilen,
                          preAligned[j]->aln->alilen);
      tmpMat = NULL;
    }
  }
...
template <typename TNODE>
void btree<TNODE>::relaxConsistMatrix(double **dist_matrix,
                                      double max_dist_cutoff,
                                      double min_cutoff) {
  int i, j, k, l;
  float **tmpMat;
  ...
  for (i = 0; i < preAligned.size(); i++) {
    for (j = i + 1; j < preAligned.size(); j++) {
      ...
      if (Debug > 1)
        cout << i << " " << j << " Number for relax: " << number_for_relax
             << endl;
      // cout << "HERE: " << endl;
      // normalize
      ...
      cout << "sum_of_elements: " << sum_of_elements << " "
           << sum_of_elements_after << " " << relaxcount << endl;
      // cout << "HERE: " << endl;
      smat1[i][j] = new sparseMatrix(tmpMat, preAligned[i]->aln->alilen,
                                     preAligned[j]->aln->alilen);
      // cout << "HERE: " << endl;
      ...
      // NEW debug
      if (debug > 1)
        cout << "Elements after relax: " << smat1[i][j]->nelements << endl;
...
void btree<TNODE>::computeConsistencyAlignment(TNODE *a) {
  ...
  path = computePairwiseAlignment(consistScoringMatrix, a->childL->absSeqlength,
                                  a->childR->absSeqlength, len);
  // free_imatrix(consistScoringMatrix, a->childL->absSeqlength,
  // a->childR->absSeqlength);
  ...
  if (debug > 1)
    cout << "Number of absSeq: " << a->childL->absSeqnum + a->childR->absSeqnum
         << endl;
  for (i = 1; i <= a->childL->absSeqnum; i++) {
    ...
    // for(j=1;j<=alignmentLength;j++) { cout << "j: " << j << "\t" <<
    // newSeq[j]; << endl; }
    if (debug > 1) {
      for (j = 1; j <= alignmentLength; j++) {
        cout << newSeq[j] << " ";
      }
      cout << endl;
    }
  ...
  for (i = a->childL->absSeqnum + 1;
       i <= a->childL->absSeqnum + a->childR->absSeqnum; i++) {
    int *newSeq = ivector(alignmentLength);
    oi = 0, ai = 0;
    if (debug > 1) cout << "===========" << endl;
    ...
    // cout << endl;
    if (debug > 1) cout << "sequence i: " << i << endl;
    // for(j=1;j<=alignmentLength;j++) { cout << "j: " << j << "\t" << newSeq[j]
    // << endl; }
    if (debug > 1)
      for (j = 1; j <= alignmentLength; j++) {
        cout << newSeq[j] << " ";
  ...
  // cout << "Number of letters for node a: " << a->absSeqnum << " * " <<
  // a->absSeqlength << " + " << a->absSeqlength*a->absSeqnum<<endl;
  if (debug > 1) cout << "Obtain the abstract alignment" << endl;
  // for(i=1;i<=a->absSeqnum;i++) { for(j=0;j<=a->absSeqlength;j++) { cout <<
  // "IJ: " << i << "\t" << j << "\t" << a->abstractSeq[i][j]<< endl; } }
  if (debug > 1) {
...
void btree<TNODE>::iterativeRefinement(int maxround) {
  ...
  // if the tree is made by UPGMA, then there is actually no need to sort, but
  // anyway sort it
...
void btree<TNODE>::recomputeAlignment(TNODE *a) {
  ...
  // cout << "partition: " << endl;
  // for(i=1;i<=root->absSeqnum;i++) { cout << "i: " << i << " " << partition[i]
  // << endl; }
  ...
  for (i = 1; i <= root->absSeqlength; i++) {
    ...
    for (j = 1; j <= root->absSeqnum; j++) {
      if (partition[j] == 1) {
        // cout << "i: " << i << " j: " << root->abstractSeq[j][i] <<"
        // "<<t1->absSeqlength<< endl;
        if (root->abstractSeq[j][i]) {
          allgap_mark1[i] = 0;
          t1->absSeqlength++;
          break;
        }
  ...
  if (debug > 1)
    for (i = 1; i <= t1->absSeqnum; i++) {
      for (j = 0; j <= t1->absSeqlength; j++) {
        cout << "t1: " << j << " " << t1->abstractSeq[i][j] << endl;
      }
    }
  ...
  int len;
  int alignmentLength = 0;
  path = computePairwiseAlignment(consistScoringMatrix, t1->absSeqlength,
                                  t0->absSeqlength, len);
  ...
  if (debug > 1)
    cout << "Number of letters for node root: " << root->absSeqnum << " * "
         << root->absSeqlength << " + " << root->absSeqlength * root->absSeqnum
         << endl;
  if (debug > 1) {
    cout << "Obtain the abstract alignment" << endl;
    // for(i=1;i<=a->absSeqnum;i++) { for(j=0;j<=a->absSeqlength;j++) { cout <<
    // "IJ: " << i << "\t" << j << "\t" << a->abstractSeq[i][j]<< endl; } }
    ...
    for (j = 1; j <= leny; j++) {
      for (k = 1; k <= a->absSeqnum; k++) {
        ...
        for (m = 1; m <= b->absSeqnum; m++) {
          ...
          // cout << "k: " << k << "\tm: " << m << "\t" << index1 << "\t" <<
          // index2 << "\t" << a->abstractSeq[k][i] << "\t" <<
          // b->abstractSeq[m][j] << endl;
          tmpMat[i][j] += smat[index1][index2]->getElement(
              a->abstractSeq[k][i], b->abstractSeq[m][j]);
          // cout << tmpMat[i][j] << endl;
        }
      }
      scoreMat[i][j] = (int)(scaling_factor * tmpMat[i][j]);
      if (scoreMat[i][j] < 0) {
        cout << "less than zero: " << scoreMat[i][j] << endl;
      }
...
template <typename TNODE>
int *btree<TNODE>::computePairwiseAlignment(int **scoreMat, int m, int n,
                                            int &len) {
  int i, j, k;
  int *path;
  ...
  cout << "maximum probablity: " << maxprobs << " " << galign.maxscore << " "
       << m << " " << n << " " << MIN(m, n) << endl;
...
void btree<TNODE>::printAlignmentFromAbs(TNODE *n, char *outfilename,
  ...
  // for(i=1;i<=n->absSeqnum;i++) {
  // preAligned[n->abstractSeq[i][0]]->aln->printali(60); } cout << "seqNUM: " <<
  // seqNUM << endl;
  tmpSeq->aseq =
      new char *[seqNUM + 1];  // cmatrix(0, seqNUM-1, 0, n->absSeqlength);
  ...
  int S = 0;
  for (i = 1; i <= n->absSeqnum; i++) {
    S += preAligned[n->abstractSeq[i][0]]->aln->nal;
  }
  tmpSeq->nal = S;
  ...
  /*
  for(i=1;i<=n->absSeqnum;i++) {
     if(tmpSeq->mnamelen<preAligned[n->abstractSeq[i][0]]->aln->mnamelen)
  tmpSeq->mnamelen = preAligned[n->abstractSeq[i][0]]->aln->mnamelen;
     for(j=1;j<=preAligned[n->abstractSeq[i][0]]->aln->nal;j++) {
          ...
          for(k=1;k<=n->absSeqlength;k++) {
                  if(n->abstractSeq[i][k]==0) {
                          ...
                          seqStr[k-1] =
  preAligned[n->abstractSeq[i][0]]->aln->aseq[j-1][n->abstractSeq[i][k]-1]; l++;
                  }
          }
          seqStr[k-1]='\0';
          assert(k-1==n->absSeqlength);
          strcpy(tmpSeq->aname[S],
  preAligned[n->abstractSeq[i][0]]->aln->aname[j-1]);
          //cout << left << setw(30) << tmpSeq->aname[S] << tmpSeq->aseq[S] <<
  endl;
  ...
  */
  for (j = 0; j < preAligned.size(); j++) {
    ...
    if (tmpSeq->mnamelen < preAligned[j]->aln->mnamelen)
      tmpSeq->mnamelen = preAligned[j]->aln->mnamelen;
    for (m = 1; m <= preAligned[j]->aln->nal; m++) {
      ...
      for (k = 1; k <= n->absSeqlength; k++) {
        if (n->abstractSeq[i][k] == 0) {
          ...
          seqStr[k - 1] =
              preAligned[j]->aln->aseq[m - 1][n->abstractSeq[i][k] - 1];
        }
      }
      seqStr[k - 1] = '\0';
      assert(k - 1 == n->absSeqlength);
  ...
  */
    ...
    if (preAligned[i]->similarSet) {
      ...
      for (j = 0; j < preAligned[i]->similarSet->nal; j++) {
        for (k = 0; k < tmpSeq->nal; k++) {
          if (strcmp(preAligned[i]->similarSet->aname[j], tmpSeq->aname[k]) ==
              0) {
            found = 1;
            break;
          }
      ...
      // tmpSeq1 = merge_align_by_one_sequence_insert(tmpSeq,
      // preAligned[i]->similarSet, tmpSeq->aname[k]); delete tmpSeq;
      //***old *** for(k=0;k<=tmpSeq->nal;k++) {delete [] tmpSeq->aseq[k];
      //delete [] tmpSeq->aname[k];}
  ...
  if (similarset_aln.size()) {
    tmpSeq1 =
        merge_master_and_slaves(tmpSeq, similarset_aln, similarset_repnames);
    delete tmpSeq;
    tmpSeq = tmpSeq1;
  ...
  if(!exclude_similar) {
          for(i=0;i<similaraln.size();i++) {
                  tmpSeq1 = merge_align_by_one_sequence_insert(tmpSeq,
  similaraln[i], repnames[i]); delete tmpSeq; tmpSeq = tmpSeq1;
          }
  }
  */
  ...
  fflush(logfp);
  if (Debug > 1) {
    cout << "alignment after refine_align_new, step 1" << endl;
    tmpSeq->printali(70);
  }
  ...
  delete_complete_gap_positions(tmpSeq);
  if (Debug > 1) {
    cout << "alignment after refine_align_new, step 2" << endl;
    tmpSeq->printali(70);
  }
  ...
  if (Debug > 1) {
    cout << "alignment after alignrefine, step 3" << endl;
    tmpSeq->printali(70);
  }
  ...
  if (tmpSeq_ss->mnamelen < strlen("Consensus_ss: "))
    tmpSeq_ss->mnamelen = strlen("Consensus_ss: ");
  ...
  // a re-implementation, finding the indexes of the representatives and use
  // them as marks of boundaries
  ...
  for (i = 0; i < preAligned.size(); i++) {
    ...
    for (k = 0; k < tmpSeq->alilen; k++) {
      if (tmpSeq->aseq[j][k] == '-') {
        ...
        tmpSeq_ss->aseq[tmpSeqnum][k] =
            ssint2ss(preAligned[i]->aln->ss->sstype[tmppos + 1]);
    ...
    for(k=0;k<preAligned[i]->similarSet->nal;k++) {
            if(strcmp(preAligned[i]->similarSet->aname[k], tmpSeq->aname[j])==0)
    continue; for(l=0;l<tmpSeq->nal;l++) { if(strcmp(tmpSeq->aname[l],
    preAligned[i]->similarSet->aname[k])==0) break;
            }
            strncpy(tmpSeq_ss->aseq[tmpSeqnum], tmpSeq->aseq[l],
    tmpSeq_ss->alilen); strcpy(tmpSeq_ss->aname[tmpSeqnum], tmpSeq->aname[l]);
  ...
  for (i = 0; i < tmpSeq->alilen; i++) {
    totalcounts = consensus_counts[i][1] + consensus_counts[i][2] +
                  consensus_counts[i][3];
    ...
    if (consensus_counts[i][1] * 1.0 / totalcounts >= 0.5)
      consensus_ss[i] = 'h';
    else if (consensus_counts[i][2] * 1.0 / totalcounts >= 0.5)
      consensus_ss[i] = 'e';
    else
      consensus_ss[i] = '.';
  }
  ...
  // now re-modified the sequences to remove small letters (which are added to
  // representatives)
  ...
  for (i = 0; i < tmpSeq_ss->nal; i++) {
    ...
    for (j = 0; j < tmpSeq_ss->alilen; j++) {
      if ((tmpSeq_ss->aseq[i][j] >= 'a') && (tmpSeq_ss->aseq[i][j] <= 'z')) {
        tmpSeq_ss->aseq[i][j] = '-';
        if (strcmp(tmpSeq_ss->aname[i - 1], "ss:") ==
            0) {  // also modify the secondary structure
  ...
  if (!exclude_similar) {
    ...
    /*
    for(i=0;i<similaraln.size();i++) {
            tmpSeq1 = merge_align_by_one_sequence_insert(tmpSeq, similaraln[i],
    repnames[i]); delete tmpSeq; tmpSeq = tmpSeq1;
    }
    for(i=0;i<similaraln.size();i++) {
            tmpSeq1 = merge_align_by_one_sequence_insert(tmpSeq_ss,
    similaraln[i], repnames[i]); delete tmpSeq_ss; tmpSeq_ss = tmpSeq1;
    }
    */
  ...
  // when adding similaraln, some gap characters "-" could be introduced into
  // the consensus_ss line, replace them with "."
  for (i = 0; i < tmpSeq_ss->nal; i++) {
...
void btree<TNODE>::printAlignmentFromAbs(TNODE *n) {
    ...
    for (j = 1; j <= preAligned[n->abstractSeq[i][0]]->aln->nal; j++) {
      ...
      for (k = 1; k <= n->absSeqlength; k++) {
        if (n->abstractSeq[i][k] == 0) {
          ...
          seqStr[k - 1] = preAligned[n->abstractSeq[i][0]]
                              ->aln->aseq[j - 1][n->abstractSeq[i][k] - 1];
          l++;
        }
      }
      seqStr[k - 1] = '\0';
      ...
      strcpy(tmpSeq->aname[S],
             preAligned[n->abstractSeq[i][0]]->aln->aname[j - 1]);
      // cout << left << setw(20) << tmpSeq->aname[S] << tmpSeq->aseq[S] <<
      // endl;
  ...
  for (l = 1; l <= n->absSeqnum; l++) {
    ...
    if (preAligned[i]->similarSet) {
      ...
      tmpSeq1 = merge_align_by_one_sequence(tmpSeq, preAligned[i]->similarSet,
                                            tmpSeq->aname[k]);
      // delete tmpSeq;
      tmpSeq = tmpSeq1;
    }
