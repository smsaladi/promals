...
void hmm_parameters::read_parameters(char *filename) {
  ...
  if (!fp) {
    cout << filename << " not readable" << endl;
    exit(0);
  }
  ...
  if (num_ss != ss1) {
    cout << "numbers of secondary structure types do not match" << endl;
    exit(0);
  }
  ...
  if (num_solv != solv1) {
    cout << "numbers of solvent accessibility categories do not match" << endl;
    exit(0);
  }
  ...
  if (unaligned1 != unaligned) {
    cout << "numbers of unaligned match states do not match" << endl;
    exit(0);
  }
  ...
  for (i = 1; i <= num_match_states; i++) {
    for (j = 1; j <= 20; j++) {
      for (k = 1; k <= 20; k++) {
        m_lor[i][j][k] =
            log(match_emission_matrix[i][j][k] / background_frequencies[j] /
                background_frequencies[k]);
  ...
  for (i = 0; i < num_states; i++) {
    ...
    // if(i==0) { for(j=1;j<=num_match_states+2;j++) t_log[i][j] =
    // log(1.0/num_states); }
  }
...
void hmm_parameters::print_parameters() {
  ...
  for (i = 1; i <= 20; i++) {
    cout << am[i] << " " << indel_emission_vector1[i] << " " << indel_lor1[i]
         << endl;
  }
  cout << endl;
  cout << "Emission vector for indel state 2:" << endl;
  for (i = 1; i <= 20; i++) {
    cout << am[i] << " " << indel_emission_vector2[i] << " " << indel_lor2[i]
         << endl;
  }
  cout << endl;
  cout << setprecision(4);
...
void hmm_multim::viterbi(int lor) {
    ...
    for (j = 0; j <= leny; j++) {
      ...
      for (k = 1; k <= num_match_states; k++) {
        if ((i == 0) || (j == 0)) {
          ...
          T[k][i][j] = -1000;
          if (debug > 1)
            fprintf(stdout, "%d %d %d:  %7.3f    %d", k, i, j, V[k][i][j],
                    T[k][i][j]);
        ...
        if ((i == 1) && (j == 1)) {
          ...
          T[k][i][j] = 0;
          if (lor == 1)
            V[k][i][j] += p->m_lor[k][ax[i]][ay[j]];
          else
            V[k][i][j] += p->m_log[k][ax[i]][ay[j]];
          if (debug > 1)
            fprintf(stdout, "%d %d %d:  %7.3f    %d", k, i, j, V[k][i][j],
                    T[k][i][j]);
          if (debug > 1) cout << endl;
        ...
        if (lor == 1)
          V[k][i][j] = p->m_lor[k][ax[i]][ay[j]] + (*it);
        else
          V[k][i][j] = p->m_log[k][ax[i]][ay[j]] + (*it);
        vv.erase(++it, vv.end());
        T[k][i][j] = vv.size();
        if (debug > 1)
          fprintf(stdout, "%d %d %d:  %7.3f    %d", k, i, j, V[k][i][j],
      ...
      if (i == 0) {
        ...
        if ((j == 0) && (i == 1)) {
          ...
          T[p1][i][j] = 0;
          if (lor == 1)
            V[p1][i][j] += p->indel_lor1[ax[i]];
          else
            V[p1][i][j] += p->indel_log1[ax[i]];
          if (debug > 1)
            fprintf(stdout, "%d %d %d:  %7.3f    %d", p1, i, j, V[p1][i][j],
                    T[p1][i][j]);
          if (debug > 1) cout << endl;
        } else {
          vv.clear();
          for (m = 1; m <= num_match_states + 2; m++) {
            ...
            // cout << p1 << " " << m << " " << i-1 << " " << j  << " " <<
            // p->t_log[m][p1] << " " << V[m][i-1][j] << endl;
          }
          it = max_element(vv.begin(), vv.end());
          if (lor == 1)
            V[p1][i][j] = p->indel_lor1[ax[i]] + (*it);
          else
            V[p1][i][j] = p->indel_log1[ax[i]] + (*it);
          vv.erase(++it, vv.end());
          T[p1][i][j] = vv.size();
          // fprintf(stdout, "%d %d %d:  %7.3f    %d", p1, i, j, V[p1][i][j],
          // T[p1][i][j]); cout << endl;
      ...
      if (j == 0) {
        ...
        if ((i == 0) && (j == 1)) {
          ...
          if (lor == 1)
            V[p2][i][j] += p->indel_lor2[ay[j]];
          else
            V[p2][i][j] += p->indel_log2[ay[j]];
          T[p2][i][j] = -1000;
          if (debug > 1)
            fprintf(stdout, "%d %d %d:  %7.3f    %d", p2, i, j, V[p2][i][j],
                    T[p2][i][j]);
          if (debug > 1) cout << endl;
        } else {
          vv.clear();
          for (m = 1; m <= num_match_states + 2; m++) {
            ...
            // cout << m << " " << p2 << " " << i << " " << j-1  << " " <<
            // p->t_log[m][p2] << " " << V[m][i][j-1] << endl;
          }
          // for(it=vv.begin();it<vv.end();it++) { cout << *it << " "; }
          // cout << endl;
          it = max_element(vv.begin(), vv.end());
          if (lor == 1)
            V[p2][i][j] = p->indel_lor2[ay[j]] + (*it);
          else
            V[p2][i][j] = p->indel_log2[ay[j]] + (*it);
          vv.erase(++it, vv.end());
          T[p2][i][j] = vv.size();
          if (debug > 1)
            fprintf(stdout, "%d %d %d:  %7.3f    %d", p2, i, j, V[p2][i][j],
  ...
  for (m = 1; m <= num_match_states + 2; m++) {
    ...
    if (debug > 1)
      cout << m << " " << V[m][lenx][leny] << " " << p->t_log[m][p_end] << " "
           << V[m][lenx][leny] + p->t_log[m][p_end] << endl;
  }
  it = max_element(vv.begin(), vv.end());
  VE = (*it);
  vv.erase(++it, vv.end());
    ...
    for (j = 0; j <= leny; j++) {
      ...
      for (k = 1; k <= p->num_match_states; k++) {
        ...
        if ((i == 1) && (j == 1)) {
          ...
          if (debug > -1)
            fprintf(stdout, "%d %d %d:  %7.3f", k, i, j, F[k][i][j]);
          if (debug > -1) cout << endl;
          continue;
        }
        F[k][i][j] = LOG_ZERO;
      ...
      if (i == 0) {
        ...
        if ((j == 0) && (i == 1)) {
          ...
          if (debug > -1)
            fprintf(stdout, "%d %d %d:  %7.3f", p1, i, j, F[p1][i][j]);
          if (debug > -1) cout << endl;
        } else {
          F[p1][i][j] = LOG_ZERO;
          for (m = 1; m <= p->num_match_states + 2; m++) {
            F[p1][i][j] =
                LOG_ADD(F[p1][i][j], p->t_log[m][p1] + F[m][i - 1][j]);
          }
          F[p1][i][j] += p->indel_log1[ax[i]];
          if (debug > -1)
            fprintf(stdout, "%d %d %d:  %7.3f", p1, i, j, F[p1][i][j]);
      ...
      if (j == 0) {
        ...
        if ((i == 0) && (j == 1)) {
          ...
          if (debug > -1)
            fprintf(stdout, "%d %d %d:  %7.3f", p2, i, j, F[p2][i][j]);
          if (debug > -1) cout << endl;
        } else {
          F[p2][i][j] = LOG_ZERO;
          for (m = 1; m <= p->num_match_states + 2; m++) {
            F[p2][i][j] =
                LOG_ADD(F[p2][i][j], p->t_log[m][p2] + F[m][i][j - 1]);
          }
          F[p2][i][j] += p->indel_log2[ay[j]];
          if (debug > -1)
            fprintf(stdout, "%d %d %d:  %7.3f", p2, i, j, F[p2][i][j]);
    ...
    for (j = leny; j >= 0; j--) {
      ...
      for (k = 1; k <= p->num_match_states + 2; k++) {
        if ((i == lenx) && (j == leny)) {
          ...
          if (debug > -1)
            cout << k << "\t" << i << "\t" << j << "\t" << B[k][i][j] << endl;
          continue;
        }
        B[k][i][j] = LOG_ZERO;
        if (i == lenx) {
          B[k][i][j] = LOG_ADD(B[k][i][j], p->t_log[k][p2] + B[p2][i][j + 1] +
                                               p->indel_log2[ay[j + 1]]);
          if (debug > -1)
            cout << k << "\t" << i << "\t" << j << "\t" << B[k][i][j] << endl;
          continue;
        ...
        if (j == leny) {
          B[k][i][j] = LOG_ADD(B[k][i][j], p->t_log[k][p1] + B[p1][i + 1][j] +
                                               p->indel_log1[ax[i + 1]]);
          if (debug > -1)
            cout << k << "\t" << i << "\t" << j << "\t" << B[k][i][j] << endl;
          continue;
        ...
        for (m = 1; m <= num_match_states; m++) {
          // cout << m << "\t" << p->t_log[k][m] << "\t" << B[m][i+1][j+1] <<
          // "\t" << p->m_log[k][ax[i+1]][ay[j+1]] << endl;
          B[k][i][j] =
              LOG_ADD(B[k][i][j], p->t_log[k][m] + B[m][i + 1][j + 1] +
                                      p->m_log[m][ax[i + 1]][ay[j + 1]]);
        }
        B[k][i][j] = LOG_ADD(B[k][i][j], p->t_log[k][p1] + B[p1][i + 1][j] +
                                             p->indel_log1[ax[i + 1]]);
        B[k][i][j] = LOG_ADD(B[k][i][j], p->t_log[k][p2] + B[p2][i][j + 1] +
  ...
  for (i = 1; i <= lenx; i++) {
    for (j = 1; j <= leny; j++) {
      ...
      for (k = 1; k <= useful_num_match_states; k++) {
        aligned_pair_prob[i][j] =
            LOG_ADD(aligned_pair_prob[i][j], F[k][i][j] + B[k][i][j] - FE);
      }
      probMat[i][j] = exp(aligned_pair_prob[i][j]);
      if (debug > -2) {
        if ((probMat[i][j] > -0.01) && (probMat[i][j] < 1.1))
          if (debug > -1)
            fprintf(stdout, "%d %d: %c %c %f %f %f\n", i, j, x->aseq[0][i - 1],
                    y->aseq[0][j - 1], aligned_pair_prob[i][j], probMat[i][j],
                    exp(F[num_match_states][i][j] + B[num_match_states][i][j] -
                        FE));
      }
    }
  }
