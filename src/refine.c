...
alignrefine::alignrefine(subalign *x, int NC_added_gaps) {
  ...
  aseq = new char *[nal];
  for (i = 0; i < nal; i++)
    aseq[i] = new char[alilen + NC_added_gaps + NC_added_gaps + 1];
  for (i = 0; i < nal; i++) {
    for (j = 0; j < NC_added_gaps; j++) aseq[i][j] = '-';
    for (j = NC_added_gaps; j < NC_added_gaps + alilen; j++)
      aseq[i][j] = a->aseq[i][j - NC_added_gaps];
    for (j = NC_added_gaps + alilen; j < NC_added_gaps + NC_added_gaps + alilen;
         j++)
      aseq[i][j] = '-';
...
alignrefine::~alignrefine() {
  int i;

  if (aseq) free_cmatrix(aseq, nal - 1, alilen);
  if (gi) free_gmatrix<gapinfo>(gi, nal, alilen);
  if (sw) delete[] sw;
  if (aaw) delete[] aaw;
  if (sdw) delete[] sdw;
...
void alignrefine::set_gappy_threshold(double g) { gappy_threshold = g; }

void alignrefine::set_CORE_POS_NUMBER(int n) { CORE_POS_NUMBER = n; }
...
void alignrefine::assign_gi() {
  ...
  for (i = 1; i <= nal; i++) {
    for (j = 1; j <= alilen; j++) {
      ...
      if ((j <= alilen - 2) && (gi[i][j].aa != '-') &&
          (aseq[i - 1][j] != '-') && (gi[i][j].lg) &&
          (aseq[i - 1][j + 1] == '-')) {
        gi[i][j].doublet = 1;
      }
      if ((j == 1) && (j <= alilen - 2) && (gi[i][j].aa != '-') &&
          (aseq[i - 1][j] != '-') && (aseq[i - 1][j + 1] == '-')) {
        gi[i][j].doublet = 1;
      }
      if ((j == alilen - 1) && (gi[i][j].aa != '-') &&
          (aseq[i - 1][j] != '-') && (gi[i][j].lg)) {
        gi[i][j].doublet = 1;
      }

      if (debug_here > 1)
        cout << i << " " << j << " " << gi[i][j].aa << " " << gi[i][j].lg << " "
             << gi[i][j].rg << " " << gi[i][j].single << " " << gi[i][j].doublet
             << endl;
    }
  }
}
...
// w_aa: a vector of lists of 21 weight values for amino acids + gap character
// for positions
void alignrefine::calculate_weights() {
  ...
  for (j = 1; j <= alilen; j++) {
    ...
    if (aaw[j] < gappy_threshold)
      isgappy.push_back(1);
    else
      isgappy.push_back(0);
    vaaw.push_back(aaw[j]);
    if (debug_here > 1)
      cout << j << ": " << aaw[j] << " " << sdw[j] << " " << isgappy[j] << endl;
  }
}
...
void alignrefine::operation() {
  ...
  for (i = 1; i <= alilen; i++) {
    // cout << sdw[i] << " " << totalweight << " " << sdw[i]/totalweight << " "
    // << a->aseq[0][i-1]<< endl;
...
void alignrefine::operation_together(int p) {
  ...
  */
  ...
  for (i = 1; i <= nal; i++) {
    ...
    if (gi[i][p].lg == 0)
      continue;  // ignore positions where the left position is not a gap
    if (p - gi[i][p].lg == 1)
      continue;  // ignore positions with a N-temrinal gap
    found = 0;
  ...
  for (i = 1; i <= nal; i++) {
    if (gi[i][p].aa == '-') continue;  // ignore gaps
    if (p + gi[i][p].rg == alilen)
      continue;  // ignore positions with a C-temrinal gap
    found = 0;
    for (it = gap_pattern.begin(); it != gap_pattern.end(); ++it) {
...
void alignrefine::operation_separate(int p) {
  ...
  for (i = 1; i <= nal; i++) {
    if (gi[i][p].single) {
      ...
      // cout << i << " " << am2num(gi[i][p].aa) << " " <<
      // w_aa[p][am2num(gi[i][p].aa)] << endl;
      ...
      if (p + gi[i][p].rg <= alilen)
        if ((aaw[p - gi[i][p].lg] < aaw[p] - 0.5) &&
            (aaw[p + gi[i][p].rg] < aaw[p] - 0.5))
          continue;
    ...
    if (gi[i][p].doublet) {
      ...
      // cout << "operation separate - single: " << i << " " << p;
      if (debug_here > 1)
        cout << "operation separate - doublet: " << i << " " << p;
      if ((aaw[p - gi[i][p].lg] < aaw[p] - 0.5) &&
          (aaw[p + gi[i][p + 1].rg] < aaw[p] - 0.5))
        continue;
...
void alignrefine::shift_left(int n, int p) {
  ...
  if (t + 1 <= alilen) {
    ...
    if (t + 2 <= alilen)
      if ((gi[n][t].aa == '-') && (gi[n][t + 1].rg == 0) && (gi[n][t + 2].rg)) {
        gi[n][t + 1].doublet = 1;
      }
  }
}
...
void alignrefine::deal_with_gappy(int gb, int ge, int filter_length) {
  ...
  if (ge - gb + 1 < filter_length) {
    ...
    if (all_gap_begin != -1) {
      ...
      all_gap_end = i - 1;
      if (debug_here > 1)
        cout << "||||| " << all_gap_begin << "  " << all_gap_end << endl;
  ...
  if (debug_here > 1)
    cout << "gb: " << gb << " ge: " << ge << " alilen: " << alilen << endl;
  if (ge == alilen) return;
  deal_with_gappy(gb, ge, filter_length);
}
...
void alignrefine::delete_all_gap(int b, int e) {
  ...
  for (i = 0; i < nal; i++) {
    ...
    for (j = 1; j <= alilen; j++) {
      a->alignment[i + 1][j] = am2num(aseq[i][j - 1]);
    }
  }
  // a->aseq = aseq;
...
void alignrefine::deal_with_gappy_individual(int gb, int ge, int n, int lmr) {
  ...
  if (n == 107) {
    if (debug_here > 1) cout << "n: " << n << endl;
    if (debug_here > 1)
      cout << "left bound: " << left_bound << " " << nstr << " " << lmn << endl;
    if (debug_here > 1)
      cout << "right bound: " << right_bound << " " << cstr << " " << rmn
           << endl;
  ...
  // 4. if gappy_aa cannot be moved, push the amino acids aside depending on the
  // position of the gaps
  if (lmn + rmn == 0) {
    if (lmr == -1) mstr = add_left_gaps(gappy_aa, right_bound - left_bound + 1);
    if (lmr == 0)
      mstr = add_middle_gaps(gappy_aa, right_bound - left_bound + 1);
    ...
    for (i = left_bound; i <= right_bound; i++) {
      ...
      if (am2num(oringal_str[i - 1])) {
        aaw[i] -= sw[n];
        vaaw[i] -= sw[n];
      }
      w_aa[i][am2num(aseq[n - 1][i - 1])] += sw[n];
      if (am2num(aseq[n - 1][i - 1])) {
        aaw[i] += sw[n];
        vaaw[i] += sw[n];
      }
      if (vaaw[i] >= gappy_threshold)
        isgappy[i] = 0;
      else
        isgappy[i] = 1;
    }
  ...
  if (lmn + rmn <= gappy_aa.size()) {
    ...
    if (debug_here > 1)
      cout << gappy_aa_N << "|" << gappy_aa_C << "|" << gappy_aa_M << endl;
  ...
  for (i = 0; i < left_core_pos.size(); i++)
    both_core_pos.push_back(left_core_pos[i]);
  for (i = 0; i < right_core_pos.size(); i++)
    both_core_pos.push_back(right_core_pos[i]);
  for (i = 0; i <= lmn; i++) {
    if (debug_here > 1) cout << "i: " << i << endl;
    if (lbm - i > rmn) continue;
    before_move = tstr;
    after_move = nstr;
    for (j = 1; j <= gb - 1 - nstr.size() - i - naa.size(); j++)
      after_move += '-';
    after_move = after_move + naa + gappy_aa.substr(0, i);
    for (j = 1; j <= ge - gb + 1; j++) after_move += '-';
    after_move += gappy_aa.substr(i);
    after_move += caa;
    for (j = 1; j <= (nstr + mstr).size() - ge - caa.size() -
                         (gappy_aa.substr(i)).size();
         j++)
      after_move += '-';
    after_move += cstr;
    ...
    if (debug_here > 1)
      cout << "oc_both: " << oc_both << " " << max_oc_both << endl;
...
double alignrefine::compare_score(string a1, string a2, vector<int> b, int n) {
  ...
  if (debug_here > 1)
    cout << before_occupancy << " " << after_occupancy << endl;
