...
void hmm_local::viterbi() {
  ...
  for (i = 1; i <= lenx; i++) {
    if (!gapopen_end)
      Vx[i][0] = max2(xy + Vm[i - 1][0], xy + Vx[i - 1][0], Tx[i][0]);
    // for end gap, waive the gap opening penalty: change
    else
      Vx[i][0] = max2(mxy + Vm[i - 1][0], xy + Vx[i - 1][0], Tx[i][0]);
    if (debug > 1) cout << "Vx i 0: " << i << " " << Vx[i][0] << endl;
  }
  for (j = 1; j <= leny; j++) {
    if (!gapopen_end)
      Vy[0][j] = max2(xy + Vm[0][j - 1], xy + Vy[0][j - 1], Ty[0][j]);
    else
      Vy[0][j] = max2(mxy + Vm[0][j - 1], xy + Vy[0][j - 1], Ty[0][j]);
  ...
  for (i = 1; i <= lenx; i++) {
    for (j = 1; j <= leny; j++) {
      ...
      // mxy+Vx[i-1][j-1], mxy+Vy[i-1][j-1], Tm[i][j]); Vx[i][j] =
      // log_odds_x_d(i) + max2(mxy+Vm[i-1][j], xy+Vx[i-1][j], Tx[i][j]);
      // Vy[i][j] = log_odds_y_d(j) + max2(mxy+Vm[i][j-1], xy+Vy[i][j-1],
      // Ty[i][j]);

      Vm[i][j] = log_odds_score(i, j) + max3(mm + Vm[i - 1][j - 1],
                                             mxy + Vx[i - 1][j - 1],
                                             mxy + Vy[i - 1][j - 1], Tm[i][j]);
      // for end gap, waive the gap opening penalty: change
      if (j == leny) {
        if (!gapopen_end)
          Vx[i][j] = max2(xy + Vm[i - 1][j], xy + Vx[i - 1][j], Tx[i][j]);
        else
          Vx[i][j] = max2(mxy + Vm[i - 1][j], xy + Vx[i - 1][j], Tx[i][j]);
      } else
        Vx[i][j] = max2(mxy + Vm[i - 1][j], xy + Vx[i - 1][j], Tx[i][j]);
      // for end gap, waive the gap opening penalty: change
      if (i == lenx) {
        if (!gapopen_end)
          Vy[i][j] = max2(xy + Vm[i][j - 1], xy + Vy[i][j - 1], Ty[i][j]);
        else
          Vy[i][j] = max2(mxy + Vm[i][j - 1], xy + Vy[i][j - 1], Ty[i][j]);
      } else
        Vy[i][j] = max2(mxy + Vm[i][j - 1], xy + Vy[i][j - 1], Ty[i][j]);

      if (debug > 1)
        fprintf(stdout, "%d %d %f %f %f %d %d %d %f\n", i, j, Vm[i][j],
                Vx[i][j], Vy[i][j], Tm[i][j], Tx[i][j], Ty[i][j],
                log_odds_score(i, j));
    }
  }

  VE = E + max3(Vm[lenx][leny] + LOG(1 - 2 * theta),
                Vx[lenx][leny] + LOG(theta), Vy[lenx][leny] + LOG(theta), TE);
  ...
  while ((xi != 0) || (yj != 0)) {
    if (debug > 1)
      fprintf(stdout, "%d %d    xi: %d yj: %d Tm %d Tx %d Ty %d\n", i, mark_mxy,
              xi, yj, Tm[xi][yj], Tx[xi][yj], Ty[xi][yj]);
    if (mark_mxy == 1) {
      a = Tm[xi][yj];
      if (a == 1) {
        // reverse_path[i+1] = 0;
...
void hmm_local::printHmmAlign(int blocksize) {
  ...
  if (x->mnamelen > y->mnamelen)
    max_name_len = x->mnamelen;
  else
    max_name_len = y->mnamelen;
...
subalign *hmm_local::productViterbiAlign() {
  ...
  a->alilen = v_alilen;
  if (x->mnamelen > y->mnamelen)
    a->mnamelen = x->mnamelen;
  else
    a->mnamelen = y->mnamelen;
...
void hmm_local::forward() {
  ...
  cout << "e:" << e << "\te1:" << e1 << "\td:" << d << "\td2t1:" << d2t1
       << "\tt:" << t << "\tep:" << ep << "\tept1:" << ept1 << endl;
  ...
  for (i = 0; i <= lenx; i++) {
    ...
    if (i == 0)
      ...
      Fx[i][0] = LOG_ADD(Fx[i - 1][0] + ep, Fn2[i - 1][0] + d) + log_odds_x(i);
  }
  for (j = 0; j <= leny; j++) {
    ...
    if (j == 0)
      ...
      Fy[0][j] = LOG_ADD(Fy[0][j - 1] + ep, Fn2[0][j - 1] + d) + log_odds_y(j);
  }
  for (i = 1; i <= lenx; i++) {
    for (j = 1; j <= leny; j++) {
      Fm[i][j] = LOG_ADD(Fm[i - 1][j - 1] + d2t1, Fx[i - 1][j - 1] + ept1,
                         Fy[i - 1][j - 1] + ept1, Fn2[i - 1][j - 1] + d2t1) +
                 log_odds(i, j);
      Fx[i][j] =
          LOG_ADD(Fm[i - 1][j] + d, Fx[i - 1][j] + ep, Fn2[i - 1][j] + d) +
          log_odds_x(i);
      Fy[i][j] =
          LOG_ADD(Fm[i][j - 1] + d, Fy[i][j - 1] + ep, Fn2[i][j - 1] + d) +
          log_odds_y(j);
    }
  }
  ...
  // for(i=0;i<=lenx;i++) { for(j=0;j<=leny;j++) { cout << i << "\t" << j << ":
  // " << Fm[i][j] << "\t" << Fx[i][j] << "\t" << Fy[i][j] << "\t" <<
  // log_odds(i,j) << endl; } }

  // Fn3
  // cout << "Here: Fn3" << endl;
  // cout << "Fn3 1 0: " << LOG_ADD(e+e+d+log_odds_x(1)+t,
  // e1+log_odds_x(1)+e+e+t)<<endl;
  for (i = 0; i <= lenx; i++) {
    for (j = 0; j <= leny; j++) {
      if ((i == 0) && (j == 0)) {
        Fn3[i][j] = Fn2[0][0] + t;
      ...
      Fn3[i][j] =
          LOG_ADD(Fm[i][j] + t, Fx[i][j] + t, Fy[i][j] + t, Fn2[i][j] + t);
  ...
  for (j = 0; j <= leny; j++) {
    for (i = 0; i <= lenx; i++) {
      if (i == 0)
        ...
        Frx2[i][j] =
            LOG_ADD(Frx2[i - 1][j] + e1, Fn3[i - 1][j] + e1) + log_odds_x(i);
      ;
      Fn4[i][j] = LOG_ADD(Frx2[i][j], Fn3[i][j]) + e;
      // cout << i << "\t" << j << ": " << Frx2[i][j] << "\t" << Fn4[i][j] <<
      // endl;
    }
  }
  ...
  for (i = 0; i <= lenx; i++) {
    for (j = 0; j <= leny; j++) {
      if (j == 0)
        ...
        Fry2[i][j] =
            LOG_ADD(Fn4[i][j - 1] + e1, Fry2[i][j - 1] + e1) + log_odds_y(j);
      Fe[i][j] = LOG_ADD(Fn4[i][j] + e, Fry2[i][j] + e);
      // cout << i << "\t" << j << ": " << Fry2[i][j] << "\t" << Fe[i][j] <<
      // endl;
    }
  }
  // cout << endl;
  ...
  // cout << "FE: " << LOG_ADD(2*e1+log_odds_x(1)+log_odds_y(1)+log(4.0),
  // e1+d+log_odds_x(1)+log_odds_y(1)+log(4.0), d2t1+log_odds(1,1) ) + 4*e + t <<
  // endl;
}
...
void hmm_local::backward() {
  ...
  // Bm, Bx, By
  for (i = lenx + 1; i >= 0; i--)
    Bm[i][leny + 1] = Bx[i][leny + 1] = By[i][leny + 1] = LOG_ZERO;
  for (i = leny + 1; i >= 0; i--)
    Bm[lenx + 1][i] = Bx[lenx + 1][i] = By[lenx + 1][i] = LOG_ZERO;
  for (i = lenx; i >= 0; i--) {
    for (j = leny; j >= 0; j--) {
      Bm[i][j] = LOG_ADD(Bm[i + 1][j + 1] + log_odds(i + 1, j + 1) + d2t1,
                         Bx[i + 1][j] + log_odds_x(i + 1) + d,
                         By[i][j + 1] + log_odds_y(j + 1) + d, Bn3[i][j] + t);
      Bx[i][j] = LOG_ADD(Bm[i + 1][j + 1] + log_odds(i + 1, j + 1) + ept1,
                         Bx[i + 1][j] + log_odds_x(i + 1) + ep, Bn3[i][j] + t);
      By[i][j] = LOG_ADD(Bm[i + 1][j + 1] + log_odds(i + 1, j + 1) + ept1,
                         By[i][j + 1] + log_odds_y(j + 1) + ep, Bn3[i][j] + t);
    }
  }
  // cout << "Be Here: " << endl;
  ...
  for (i = lenx; i >= 0; i--) {
    for (j = leny; j >= 0; j--) {
      Bn2[i][j] = LOG_ADD(Bm[i + 1][j + 1] + d2t1 + log_odds(i + 1, j + 1),
                          Bx[i + 1][j] + d + log_odds_x(i + 1),
                          By[i][j + 1] + d + log_odds_y(j + 1), Bn3[i][j] + t);
    }
  }
  cout << "Be Here: " << endl;
  ...
  for (j = leny; j >= 0; j--) {
    for (i = lenx; i >= 0; i--) {
      Bry1[i][j] =
          LOG_ADD(Bry1[i][j + 1] + log_odds_y(j + 1) + e1, Bn2[i][j] + e);
    }
  }
  cout << "Be Here: " << endl;
  ...
  for (j = leny; j >= 0; j--) {
    for (i = lenx; i >= 0; i--) {
      Bn1[i][j] =
          LOG_ADD(Bry1[i][j + 1] + e1 + log_odds_y(j + 1), Bn2[i][j] + e);
    }
  }
  cout << "Be Here: " << endl;
  ...
  for (j = leny; j >= 0; j--) {
    ...
    for (i = lenx; i >= 0; i--) {
      Brx1[i][j] =
          LOG_ADD(Brx1[i + 1][j] + log_odds_x(i + 1) + e1, Bn1[i][j] + e);
    }
  }
  // cout << "Be Here: " << endl;
  ...
  // for(i=0;i<=lenx;i++) { for(j=0;j<=leny;j++) { cout << i << "\t" << j << ":
  // " << LOG_ADD(Fry1[i][j]+Bry1[i][j], Fn2[i][j]+Bn2[i][j], Fm[i][j]+Bm[i][j],
  // Fx[i][j]+Bx[i][j], Fy[i][j]+By[i][j], Fn3[i][j]+Bn3[i][j],
  // Frx2[i][j]+Brx2[i][j]) << endl; } }
  ...
  ///Bm[i][j] + Fx[i][j] * Bx[i][j] + Fy[i][j] * By[i][j]; //fprintf(stdout, "%d
  ...
  for (i = 1; i <= lenx; i++) {
    for (j = 1; j <= leny; j++) {
      ...
      ///aligned_pair_prob[i][j].INT, aligned_pair_prob[i][j].DEC,
      ///aligned_pair_prob[i][j].real());
...
void hmm_local::baumwelch() {
  ...
  float rx1rx1, rx1n1, n1ry1, n1n2, ry1ry1, ry1n2, n2m, n2x, n2y, n2n3, mm, mx,
      my, mn3, xm, xx, xn3, yy, ym, yn3, n3rx2, n3n4, rx2rx2, rx2n4, n4ry2,
      ry2ry2, n4e, ry2e, LOG_ZERO;
  rx1rx1 = rx1n1 = n1ry1 = n1n2 = ry1ry1 = ry1n2 = n2m = n2x = n2y = n2n3 = mm =
      mx = my = mn3 = xm = xx = xn3 = yy = ym = yn3 = n3rx2 = n3n4 = rx2rx2 =
          rx2n4 = n4ry2 = ry2ry2 = n4e = ry2e = -2e20;
  ...
  for (i = 1; i < lenx; i++) {
    if (i < 0) {
      ...
      if (i < lenx)
        rx1rx1 =
            LOG_ADD(rx1rx1, Frx1[i] + e1 + log_odds_x(i + 1) + Brx1[i + 1][0]);
      if (i > 0) rx1n1 = LOG_ADD(rx1n1, Frx1[i] + e + Bn1[i][0]);
    }
    // cout << i << ": " << rx1rx1 << "\t" << rx1n1 << "\t" << Frx1[i] << "\t"
    // << log_odds_x(i+1) <<"\t" <<  Brx1[i+1][0] << endl;
  }
  rx1rx1 -= BE;
  rx1n1 -= BE;
  cout << rx1rx1 << endl;
  ...
  for (i = 1; i < lenx; i++) {
    for (j = 1; j < leny; j++) {
      if (j < leny)
        ry1ry1 = LOG_ADD(ry1ry1,
                         Fry1[i][j] + e1 + log_odds_y(j + 1) + Bry1[i][j + 1]);
  ...
  for (i = 1; i < lenx; i++) {
    for (j = 1; j < leny; j++) {
      if ((i < lenx) && (j < leny)) {
        n2m = LOG_ADD(
            n2m, Fn2[i][j] + d2t1 + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1]);
        mm = LOG_ADD(
            mm, Fm[i][j] + d2t1 + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1]);
        xm = LOG_ADD(
            xm, Fx[i][j] + ept1 + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1]);
        ym = LOG_ADD(
            ym, Fy[i][j] + ept1 + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1]);
      }
      if (i < lenx) {
  ...
  for (i = 1; i < lenx; i++) {
    for (j = 1; j < leny; j++) {
      if (i < lenx) {
        n3rx2 =
            LOG_ADD(n3rx2, Fn3[i][j] + e1 + log_odds_x(i + 1) + Brx2[i + 1][j]);
        rx2rx2 = LOG_ADD(rx2rx2,
                         Frx2[i][j] + e1 + log_odds_x(i + 1) + Brx2[i + 1][j]);
      }
      if (i == lenx) {
  ...
  for (j = 1; j < leny; j++) {
    ...
    if (j < leny) {
      ry2ry2 =
          LOG_ADD(ry2ry2, Fry2[lenx][j] + e1 + log_odds_y(j + 1) + Bry2[j + 1]);
      n4ry2 =
          LOG_ADD(n4ry2, Fn4[lenx][j] + e1 + log_odds_y(j + 1) + Bry2[j + 1]);
    }
  }
  ry2ry2 -= BE;
  n4e -= BE;
  ...
  cout << "CountEP and CountEP1: " << exp(countEP) << "\t" << exp(countEP1)
       << endl;
