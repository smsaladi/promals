...
int main (int argc, char **argv) {
        ...
        sprintf(outalnfile, "%s_%2.2lf_%d_%d.clustal", argv[1], gap_thr_lowest,
use_hwt, extend_gap_region); aln->printali(outalnfile, 60);
  ...
  /* if(remove_frag) {
     if(!use_hwt) {
          ...
          for(i=1;i<=csv->alilen;i++) {
                  gap_fraction[i] = 0;
                  for(j=1;j<=csv->nal;j++) {
                          if(csv->alignment[j][i] == 0) gap_fraction[i] +=
  csv->hwt_all[j];
                  }
                  gap_fraction[i] /= total_hwt;
                  gap_fraction1[i] = gap_fraction[i];
...
void treat_single_residues(subalign *aln, double position_fraction) {
  ...
  for (j = 1; j <= alilen; j++) {
    ...
    for (i = 1; i <= nal; i++) {
      ...
      } else if ((aln->alignment[i][j + 1] == 0) &&
                 (aln->alignment[i][j - 1] == 0)) {
        sr_record[i][j] = 1;
      }
  ...
  for (j = 1; j <= alilen; j++) {
    ...
    // if the fraction of single residues exceeds position_fraction; do not
    // consider them
    if (1.0 * sr_count / nal > position_fraction) {
      for (i = 1; i <= nal; i++) sr_record[i][j] = 0;
    }
    // otherwise make the corrections --> find N, C adjacent residues, switch
    // with the gap
    else {
      for (i = 1; i <= nal; i++) {
        if (sr_record[i][j] == 1) {
          ...
          if (j == 1)
            ...
            t_pos = n_pos;
          else if (gap_fraction[n_pos] > gap_fraction[c_pos])
            t_pos = c_pos;
          else
            t_pos = n_pos;
