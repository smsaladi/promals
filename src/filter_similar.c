...
void do_filter_similar(sequences &allseqs, char *filename, float thr,
  ...
  // 1. output the sequences to a new fasta file with seq names potentially
  // changed
  char filefilter[500];
  ...
  // sprintf(command, "%s -i %s -o %s.clus -c %f -d 30 -aL 0.95 -p 1 1>/dev/null
  // 2>/dev/null", cdhit, filefilter, filefilter, thr); printinfo(command, 0);
  // system(command);
  // exit(0);
  ...
  while (fp.getline(tmpstr, 1000)) {
    ...
    *ss = '\0';
    seqname = new char[26];
    // strcpy(seqname, s);
    strncpy(seqname, s, 26);  // pay attention to strncpy ending, '\0' is not
...
// run cdhit to get the cluster files, dynamically adjust the similarity
// threshold util the number of clusters is smaller than a certain threshold
// (10000)
void run_cdhit(char *filefilter, float thr) {
  int i, j, k;
  ...
  char command[500];
  sprintf(command,
          "%s -i %s -o %s.clus -c %f -d 30 -aL 0.95 -p 1 -n %d 1>%s.info "
          "2>/dev/null",
          cdhit, filefilter, filefilter, thr, noption, filefilter);
  system(command);
  ...
  for (i = 1; i < 100; i++) {
    ...
    char command[500];
    sprintf(command,
            "%s -i %s -o %s.clus -c %f -d 30 -aL %f -p 1 -n %d 1>%s.info "
            "2>/dev/null",
            cdhit, filefilter, filefilter, coption, aLoption, noption,
            filefilter);
    system(command);
    printinfo(command, 0);
    fp.open(info, ios::in);
    ...
    fp.close();
    fp.clear();
    cout << "\tc: " << coption << " aL: " << aLoption << " n: " << noption
         << endl;
    cout << "\tnumber of clusters: " << clusternum << endl;
    if (clusternum <= 10000) {
      // cout << "number of clusters: " << clusternum << endl;
...
subalign *get_similar_aln(vector<char *> seqnames, sequences &allseqs,
  ...
  if (!ofp) {
    cout << "cannot open the individualclstrfilename "
         << individualclstrfilename << endl;
    exit(0);
  }
  int mark;
  for (i = 0; i < seqnames.size(); i++) {
  ...
  char command[500];
  sprintf(command, "%s --auto %s 1>%s.aln 2>/dev/null", mafft,
          individualclstrfilename, individualclstrfilename);
  printinfo(command, 0);
  system(command);
...
void get_rep_names(sequences allseqs, vector<subalign *> &similaraln,
  ...
  for (i = 0; i < similaraln.size(); i++) {
    found = 0;
    // for(k=1;k<=similaraln[i]->nal;k++) { cout << similaraln[i]->aname[k-1] <<
    // endl; }
    for (j = 1; j < allseqs.seq.size(); j++) {
