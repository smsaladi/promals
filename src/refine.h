...
class alignrefine {
 public:
  ...
  double *aaw;  // [1..alilen] Sum-of-weight for all non-gapped letters for
                // positions
  double gappy_threshold;
  int CORE_POS_NUMBER;
  void set_gappy_threshold(double g);
  void set_CORE_POS_NUMBER(int n);
  double *sdw;  // [1..alilen] Sum-of-weight for letters that are single letters
                // or doublets for positions
