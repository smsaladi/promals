...
void sequences::readFasta(char *inputFastaName, int zap, int disallow_one_seq) {
	...
	while(fastaFile) {
		...
		if(tmpstring[0]=='>') {
			...
			for(i=0;i<tmpstring1.length();i++) {
				string::size_type pos = disallowed_letters.find(tmpstring1.substr(i,1), 0);
				if (pos != string::npos) {
					tmpstring += '_';
				}
				else {
	...
	if(nseqs==1) {
		...
		cout << "       Please input at least two sequences for alignment construction." << endl;
		exit(0);
	}
	...
	for(i=1;i<=nseqs;i++) {
		if(name[i].size()==0) {
			cout << endl;
			cout << "Error: sequence number " << i << " does not have a name." << endl;
		...
		for(j=i+1;j<=nseqs;j++) {
			if(name[i] == name[j]) {
				cout << "sequence number " << i << " and sequence number " << j << " have the same name: " << name[i] << endl;
				same_name_check = 1;
			}
		}
	}
	if(same_name_check) { 
		...
		cout << "       - there should be no two sequences with the same name in your dataset" << endl;
	...
	for(i=1;i<=nseqs;i++) {
		if(seq[i].length()==0) {
			cout << "Error message: " << endl;
			cout << "      - The sequence with name '" << name[i] << "' has a zero length" << endl;
			zero_len_check = 1;
		}
	}
	if(zero_len_check) {
...
void sequences::zapLetters(string &str) {
	...
	for(i=0;i<str.length();i++) {
		c1 = str[i];
		switch(c1) {
			// blastpgp does not work with [oOjJ], doesnot include them in checkpoint file
			// psipred thus misses these letters and creates problems
			case 'o':str[i] = 'A'; continue;
			case 'O':str[i] = 'A'; continue;
			case 'j':str[i] = 'A'; continue;
			case 'J': str[i] = 'A'; continue;
...
void sequences::output_fasta(char *fasta_file_name) {
        ...
        if(!ofp) {
                cout << "cannot open " << fasta_file_name << " for writing fasta records" << endl;
        }
        for(i=1;i<=nseqs;i++) {
...
void sequences::get_kmer_array(int K) {
	...
	for(i=1;i<=nseqs;i++) {
		...
		for(j=1;j<=seq[i].length()-K+1;j++) {
			tmpindex = 0;
			tmpindex = n5 * tmpintseq[j-1] + n4 * tmpintseq[j] + n3 * tmpintseq[j+1] + n2 * tmpintseq[j+2] + n1 * tmpintseq[j+3] + tmpintseq[j+4];
...
void sequences::get_kmer_distance(int K) {
	...
	for(i=1;i<=nseqs;i++) {
		...
		for(j=i+1;j<=nseqs;j++) {
			commonCount = 0;
                        //if(seq[i].length() > seq[j].length() ) { minLength = seq[j].length(); } else minLength = seq[i].length();
                        minLength = MIN_HERE(seq[i].length(), seq[j].length());
                        if(minLength < 6) minLength = 6;
                        //cout << j << " minLength: " << minLength << endl;
                        //cout << "len1: " << seq[i].length() << " len2: " << seq[j].length() << endl;
                        //cout << "count: " << count << endl;
                        ...
                        //vector implementation of non-zero indexes, slower
                        for(k=0;k<a1.size();k++) {
                                commonCount += MIN_HERE(kmer_array[i][a1[k]], kmer_array[j][a1[k]]);
                        }
                        */
			...
			distMat[i][j] = log(0.1 + 1.0 * commonCount/(minLength-K+1) ) * log10;
                        //cout << "TEERERER" << endl;
