  ...
  for (i = 0; i <= lenx; i++) {
    for (j = 0; j <= leny; j++) {
      ...
      if ((i == 0) && (j == 1)) {
        // cout << tran_begin[3]<<" "<<y_alphabet[j]<<" "<< ay[j]<<" " <<
        // aa_loop[y_alphabet[j]][ay[j]]<<endl; Fy[i][j] = tran_begin[3] +
        // aa_loop[y_alphabet[j]][ay[j]]; Fy[i][j] = tran_begin[3] +
        // aa_loop[y_alphabet[j]][ay[j]];
        Fy[i][j] = tran_begin[3] + aa_loop[0][ay[j]] +
                   ss_w * ss_loop[0][y_alphabet[j]];
        // cout << i << " " << j << " " << y->aseq[0][j-1] << " " << Fy[i][j] <<
        // endl;
        Fx[i][j] = Fm[i][j] = LOG_ZERO;
        if (debug > 1)
      ...
      if ((i == 0) && (j > 1)) {
        ...
        // tran_y[y_alphabet[j-1]][2] + aa_loop[y_alphabet[j]][ay[j]]; cout <<
        // y_alphabet[j] << " " << ay[j]  << endl;
        Fy[i][j] = Fy[i][j - 1] + tran_y[0][2] + aa_loop[0][ay[j]] +
                   ss_w * ss_loop[0][y_alphabet[j]];
        Fx[i][j] = Fm[i][j] = LOG_ZERO;
        // cout << i << " " << j << " " << y->aseq[0][j-1] << " " << Fy[i][j] <<
        // endl;
      ...
      if ((i == 1) && (j == 0)) {
        // cout << tran_begin[2] << " " << aa_loop[x_alphabet[i]][ax[i]] <<
        // endl; Fx[i][j] = tran_begin[2] + aa_loop[x_alphabet[i]][ax[i]];
        Fx[i][j] = tran_begin[2] + aa_loop[0][ax[i]] +
                   ss_w * ss_loop[0][x_alphabet[i]];
        Fy[i][j] = Fm[i][j] = LOG_ZERO;
        // cout << i << " " << j << " " << Fx[i][j] << endl;
        if (debug > 1)
          cout << i << " " << j << " " << Fm[i][j] << " " << Fx[i][j] << " "
      ...
      if ((i > 1) && (j == 0)) {
        // Fx[i][j] = Fx[i-1][j] + tran_x[x_alphabet[i-1]][2] +
        // aa_loop[x_alphabet[i]][ax[i]];
        Fx[i][j] = Fx[i - 1][j] + tran_x[0][2] + aa_loop[0][ax[i]] +
                   ss_w * ss_loop[0][x_alphabet[i]];
        // cout << i << " " << j << " " << Fx[i][j] << endl;
        Fy[i][j] = Fm[i][j] = LOG_ZERO;
        if (debug > 1)
      ...
      Fx[i][j] = LOG_ADD(tran_match[alphax_1][2] + Fm[i - 1][j],
                         tran_x[0][2] + Fx[i - 1][j]) +
                 p->aa_loop[0][axi] + ss_w * ss_loop[0][alphax];
      Fy[i][j] = LOG_ADD(tran_match[alphax][3] + Fm[i][j - 1],
                         tran_y[0][2] + Fy[i][j - 1]) +
                 p->aa_loop[0][ayj] + ss_w * ss_loop[0][alphay];
...
void hmm_psipred::backward2() {
  ...
  for (i = lenx; i >= 0; i--) {
    for (j = leny; j >= 0; j--) {
      ...
      if (i == lenx) {
        // By[i][j] =
        // By[i][j+1]+tran_y[alphay][2]+aa_loop[y_alphabet[j+1]][ay[j+1]];
        By[i][j] = By[i][j + 1] + tran_y[0][2] + aa_loop[0][ay[j + 1]] +
                   ss_w * ss_loop[0][y_alphabet[j + 1]];
        // Bm[i][j] =
        // By[i][j+1]+tran_match[alphax][3]+aa_loop[y_alphabet[j+1]][ay[j+1]];
        Bm[i][j] = By[i][j + 1] + tran_match[alphax][3] +
                   aa_loop[0][ay[j + 1]] + ss_w * ss_loop[0][y_alphabet[j + 1]];
        // cout << "By[i][j]: " << i << " " << j << " " << By[i][j] << endl;
        // cout << "Bm[i][j]: " << i << " " << j << " " << Bm[i][j] << endl;
        Bx[i][j] = LOG_ZERO;
        if (debug > 1)
      ...
      if (j == leny) {
        // Bx[i][j] =
        // Bx[i+1][j]+tran_x[alphax][2]+aa_loop[x_alphabet[i+1]][ax[i+1]]+ss_w*ss_loop[0][x_alphabet[i+1]];
        // Bm[i][j] =
        // Bx[i+1][j]+tran_match[alphax][2]+aa_loop[x_alphabet[i+1]][ax[i+1]];
        Bx[i][j] = Bx[i + 1][j] + tran_x[0][2] + aa_loop[0][ax[i + 1]] +
                   ss_w * ss_loop[0][x_alphabet[i + 1]];
        Bm[i][j] = Bx[i + 1][j] + tran_match[alphax][2] +
                   aa_loop[0][ax[i + 1]] + ss_w * ss_loop[0][x_alphabet[i + 1]];
        By[i][j] = LOG_ZERO;
        // cout << "Bx[i][j]: " << i << " " << j << " " << Bx[i][j] << endl;
        // cout << "Bm[i][j]: " << i << " " << j << " " << Bm[i][j] << endl;
        if (debug > 1)
      ...
      Bm[i][j] = LOG_ADD(Bm[i + 1][j + 1] + tran_match[alphax][1] +
                             aa_pair[alphax_1][axi_1][ayj_1] +
                             ss_w * ss_pair[alphax_1][alphay_1],
                         Bx[i + 1][j] + tran_match[alphax][2] +
                             aa_loop[0][axi_1] + ss_w * ss_loop[0][alphax_1],
                         By[i][j + 1] + tran_match[alphax][3] +
                             aa_loop[0][ayj_1] + ss_w * ss_loop[0][alphay_1]);
      // cout << "Bm[i][j]: " << i << " " << j << " " << Bm[i][j] << endl;
      // cout << setprecision(10) <<
      // Bm[i+1][j+1]+tran_x[alphax][1]+aa_pair[alphax_1][axi_1][ayj_1]+ ss_w *
      // ss_pair[alphax_1][alphay_1] << " " <<
      ...
      // if(Bm[i+1][j+1]+tran_x[alphax][1]+aa_pair[alphax_1][axi_1][ayj_1]+ss_w
      // * ss_pair[alphax_1][alphay_1]==Bx[i+1][j]+tran_x[alphax][2]+aa_loop[alphax_1][axi_1]) { cout << "x equal y" << endl; }
      Bx[i][j] = LOG_ADD(Bm[i + 1][j + 1] + tran_x[0][1] +
                             aa_pair[alphax_1][axi_1][ayj_1] +
                             ss_w * ss_pair[alphax_1][alphay_1],
                         Bx[i + 1][j] + tran_x[0][2] + aa_loop[0][axi_1] +
                             ss_w * ss_loop[0][alphax_1]);
      // cout << "Bx[i][j]: " << i << " " << j << " " << Bx[i][j] << endl;
      By[i][j] = LOG_ADD(Bm[i + 1][j + 1] + tran_y[0][1] +
                             aa_pair[alphax_1][axi_1][ayj_1] +
                             ss_w * ss_pair[alphax_1][alphay_1],
                         By[i][j + 1] + tran_y[0][2] + aa_loop[0][ayj_1] +
                             ss_w * ss_loop[0][alphay_1]);
      if (debug > 1)
        cout << i << " " << j << " " << Bm[i][j] << " " << Bx[i][j] << " "
             << By[i][j] << endl;
      // cout << "By[i][j]: " << i << " " << j << " " << By[i][j] << endl;
  ...
  BE = LOG_ADD(Bm[1][1] + tran_begin[1] + aa_pair[x_alphabet[1]][ax[1]][ay[1]] +
                   ss_w * ss_pair[x_alphabet[1]][y_alphabet[1]],
               Bx[1][0] + tran_begin[2] + aa_loop[0][ax[1]] +
                   ss_w * ss_loop[0][x_alphabet[1]],
               By[0][1] + tran_begin[3] + aa_loop[0][ay[1]] +
                   ss_w * ss_loop[0][y_alphabet[1]]);
