...
void constraint::assign_originalseq(sequences *os) {
        oseq = os;
}
...
// -3: redundant sequences (only one sequence is selected for each prealigned group)
// inside_index:
// 0: representative sequence itself
// >0: non-representative sequence
// -1: when rep_index is <0
void constraint::checkNamesSequences() {
        ...
        for(i=1;i<=nseqs;i++) {
                if(rep_index[i] == -1) {
                        cout << "The sequence with name " << name[i] << " does not match any names in original sequences" << endl;
                        continue;
                }
                ...
                else a1 = removegap(tn->similarSet->aseq[inside_index[i]-1], 1);
                ...
                if( loc != string::npos ) {
                        ...
                        cout << "the sequence with name " << name[i] << " does not match the one in the original sequence" << endl;
                        cout << a1 << endl;
                        cout << a2 << endl;
                        startp[i] = -1;
                        rep_index[i] = -2;
        ...
        // remove redundancy: representatives (inside_index: 0) are in priority, otherwise,
        // remove redundancy: the first occurance is kept, the others removed for 
        // sequences within the same prealigned group
        ...
        // keep one non-representative sequence (the first one) if representative sequence
        // is not available
        for(i=1;i<=nseqs;i++) {
                if(rep_index[i] < 0) continue;
...
void constraint::printIndexSeq() {
        for(int i=1;i<=nseqs;i++) {
                fprintf(stdout, "%20s %3d %3d %3d %s\n", name[i].c_str(), rep_index[i], inside_index[i], startp[i], seq[i].c_str());
        }
...
string constraint::removegap(string a1, int touppercase) {
        ...
        for(i=0;i<a1.size();i++) {
                if((a1[i]!='-') && (a1[i]!='.') && (a1[i]!=' ')) {
                        if(touppercase) a2 += toupper(a1[i]);
                        else a2 += a1[i];
                        }
        }
        return a2;
...
float ** constraint::get_constraint_matrix(int ci, int cj) {
        ...
        for(i=1;i<=replen1;i++) for(j=1;j<=replen2;j++) score_matrix[i][j] = 0;
        ...
        if(inside_index[ci]!=0) {
                ...
                for(i=0;i<(*prealigned)[rep_index[ci]]->similarSet->nal;i++) {
                        if(strcmp((*prealigned)[rep_index[ci]]->aln->aname[0], (*prealigned)[rep_index[ci]]->similarSet->aname[i])==0) ind1 = i;
                }
                tmpseq1 = (*prealigned)[rep_index[ci]]->similarSet->aseq[inside_index[ci]-1];
        ...
        if(inside_index[cj]!=0) {
                ...
                for(i=0;i<(*prealigned)[rep_index[cj]]->similarSet->nal;i++) {
                        if(strcmp((*prealigned)[rep_index[cj]]->aln->aname[0], (*prealigned)[rep_index[cj]]->similarSet->aname[i])==0) ind2 = i;
                }
                tmpseq1 = (*prealigned)[rep_index[cj]]->similarSet->aseq[inside_index[cj]-1];
