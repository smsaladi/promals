...
void big_pass(int M,int N,int K,int nseq);
void locate(int nseq);
void small_pass(int count,int nseq);
...
char *lckalloc(int amount);			/* space-allocating function */
#ifndef FAR_PTR
...
void SIM(int M,int N,int K,int **V,int Q,int R,int nseq,int gi, int gj)
{
  ...
  if (nseq == 2) for ( i = 1; i <= M; i++ ) row[i]= PAIRNULL;
	  ...
	  z = ( pairptr )FCKALLOC((int)sizeof(pair_here)*M);
	  for ( i = 1; i <= M; i++,z++) {
		  row[i] = z;
		  z->COL = i;			
		  z->NEXT = PAIRNULL;
  ...
  for ( i = 0; i < K ; i++ )
    ...
    LIST[i] = (vertexptr ) ckalloc(sizeof(vertex));
  ...
  for ( count = K - 1; count >= 0 ; count-- )
	...
	/*for(i=0;i<rl;i++) {
      ...
      aln_path1 = (int *) ckalloc( (al_len+1) * sizeof(int));
      aln_path2 = (int *) ckalloc( (al_len+1) *sizeof(int) );
    ...
    { if (M <= 0)
	  ...
	  */
	  /* mark (A[I],B[J]) as used: put J into list row[I] */	
          I++; J++;
	  z = ( pairptr ) FCKALLOC( (int) sizeof(pair_here));
          z->COL = J;			
          z->NEXT = row[I];				
	  row[I] = z;
...
void lfatalf(char *msg, char *val)
{
...
char *lckalloc(int amount)
{
	char *malloc(size_t a), *p;
	static long mtotal;
