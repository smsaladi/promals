...
void read_regularizer(char *file_name) {
	...
	while(fp.good() ) {
		...
		fp >> line;
		if(strncmp(line, "sn:", 3)==0) { fp >> line; fp >> rsn; }
		if(strncmp(line, "sb:", 3)==0) { fp >> line; fp >> rsb; } 
		if(strncmp(line, "nb:", 3)==0) { fp >> line; fp >> rnb; } 
		if(strncmp(line, "nn:", 3)==0) { fp >> line; fp >> rnn; } 
		if(strncmp(line, "ec:", 3)==0) { fp >> line; fp >> rec; } 
		if(strncmp(line, "et:", 3)==0) { fp >> line; fp >> ret; } 
		if(strncmp(line, "cc:", 3)==0) { fp >> line; fp >> rcc; } 
		if(strncmp(line, "ct:", 3)==0) { fp >> line; fp >> rct; } 
		if(strncmp(line, "bm:", 3)==0) { break; } 
	}

	// bm and me
	j = 10;
	for(i=1;i<=j;i++) { fp >> line; fp >> line; fp >> rbm[i]; }
	if(j==10) { fp >> line; fp >> rbm[11]; }
	while(fp.good() ){
		fp >> line;
		if(strncmp(line, "me:", 3)==0) { break; }
	}
	for(i=1;i<=j;i++) { fp >> line; fp >> line; fp >> rme[i]; }
	if(j==10) { fp >> line; fp >> rme[11]; }
	...
	while(fp.good() ) {
		...
		if(strcmp(line, "env")==0) {
			fp >> line; fp >> line; fp >> line; fp >> line;
			fp >> rtrans[j][1][1];
			fp >> rtrans[j][1][2];
			fp >> rtrans[j][1][3];
			fp >> line; fp >> line; fp >> line;
			fp >> rtrans[j][2][1];
			fp >> rtrans[j][2][2];
			fp >> rtrans[j][2][3];
			fp >> line; fp >> line; fp >> line;
...
void print_regularizer() {
	...
	cout << "rbm:" << endl;
	for(i=1;i<=11;i++) { cout << i << "  " << rbm[i] << endl; }

	cout << "rme:" << endl;
	for(i=1;i<=11;i++) { cout << i << "  " << rme[i] << endl; }
