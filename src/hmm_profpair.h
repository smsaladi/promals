...
class hmm_profpair {
 public:
  ...
  // viterbi
  float **Vm, **Vx, **Vy, VE;  // maximum logrithm probability ending with m, x,
                               // or y or the last one E
  int **Tm, **Tx, **Ty, TE;    // tracking
  int *path, *path1,
      *path2;    // path: 0, aligned position; 1, x to -; -1: - to y
  ...
  float max3(float a, float b, float c,
             int &t);                    // t stores the index of the max
  ...
  subalign *
  productViterbiAlign();  // generate an alignment of the two alignments
