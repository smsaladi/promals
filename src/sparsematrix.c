...
sparseMatrix::sparseMatrix(float **regularMat, int m, int n) {
  ...
  // else { cout << "crs in sparseMatrix must be specified as 1 or -1" << endl;
  // exit(0); }
...
void sparseMatrix::printCrs(char *seq1, char *seq2) {
  int i, j;

  cout << "Number of elements in sparse matrix: " << nelements << endl;
  for (i = 1; i <= nrows; i++) {
    cout << "Row number " << i << " " << seq1[i - 1] << " :" << endl;
    for (j = rstart[i - 1]; j < rstart[i]; j++) {
      cout << "\t" << rindex[j] << "\t" << seq2[rindex[j] - 1] << "\t"
           << rvalue[j] << endl;
    }
  }
}
...
void sparseMatrix::multiplyConstant(float c) {
  ...
  */
  if (rvalue)
    for (i = 0; i < nelements; i++) rvalue[i] *= c;
  if (cvalue)
    for (i = 0; i < nelements; i++) cvalue[i] *= c;
}
