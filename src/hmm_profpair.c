...
void hmm_profpair::forward() {
    ...
    for (j = 0; j <= leny; j++) {
      ...
      // else if( (i==0)&&(j==1) ) {
        ...
        if (j == 1) {
          ...
          Fy[i][j] =
              LOG_ADD(xy + Fm[i][j - 1], xy + Fy[i][j - 1]) + log_odds_y(j);
      }
      // else if( (i==1)&&(j==0) ) {
      else if ((i != 0) && (j == 0)) {
        Fm[i][j] = LOG_ZERO;
        ...
        if (i == 1) {
          ...
          Fx[i][j] =
              LOG_ADD(xy + Fm[i - 1][j], xy + Fx[i - 1][j]) + log_odds_x(i);
      } else {
        ////Fm[i][j] = mm * Fm[i-1][j-1] + mxy * Fx[i-1][j-1] + mxy *
        ///Fy[i-1][j-1]; /Fm[i][j] = Fm[i][j] * log_odds(i,j);
        if ((i == 1) && (j == 1)) {
          Fm[i][j] = LOG(1 - 2 * theta) + log_odds(i, j);
        } else
          Fm[i][j] = LOG_ADD(mm + Fm[i - 1][j - 1], mxy + Fx[i - 1][j - 1],
                             mxy + Fy[i - 1][j - 1]) +
                     log_odds(i, j);
        // waive gap open penalty for ends
  ...
  FE = E + LOG_ADD(Fm[lenx][leny] + LOG(1 - 2 * theta),
                   Fx[lenx][leny] + LOG(theta), Fy[lenx][leny] + LOG(theta));
...
void hmm_profpair::backward() {
  ...
  for (i = lenx; i >= 0; i--) {
    for (j = leny; j >= 0; j--) {
      ...
      */
      ////Bm[i][j] = mm * log_odds(i+1,j+1) * Bm[i+1][j+1] + xym *
      ///(log_odds_x(i+1) * Bx[i+1][j] + log_odds_y(j+1) * By[i][j+1]);
      Bm[i][j] = LOG_ADD(mm + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1],
                         xym + LOG_ADD(log_odds_x(i + 1) + Bx[i + 1][j],
                                       log_odds_y(j + 1) + By[i][j + 1]));

      if ((j == 0) && (i == 0)) {
        ////Bx[i][j] = xy * log_odds(i+1,j+1) * Bm[i+1][j+1] + xy *
        ///log_odds_x(i+1) * Bx[i+1][j];
        Bx[i][j] = LOG_ADD(xy + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1],
                           xy + log_odds_x(i + 1) + Bx[i + 1][j]);
      } else
        ////Bx[i][j] = mxy * log_odds(i+1,j+1) * Bm[i+1][j+1] + xy *
        ///log_odds_x(i+1) * Bx[i+1][j];
        Bx[i][j] = LOG_ADD(mxy + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1],
                           xy + log_odds_x(i + 1) + Bx[i + 1][j]);
      if ((j == 0) && (i == 0)) {
        ////By[i][j] = xy * log_odds(i+1,j+1) * Bm[i+1][j+1] + xy *
        ///log_odds_y(j+1) * By[i][j+1];
        By[i][j] = LOG_ADD(xy + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1],
                           xy + log_odds_y(j + 1) + By[i][j + 1]);
      } else
        ////By[i][j] = mxy * log_odds(i+1,j+1) * Bm[i+1][j+1] + xy *
        ///log_odds_y(j+1) * By[i][j+1];
        By[i][j] = LOG_ADD(mxy + log_odds(i + 1, j + 1) + Bm[i + 1][j + 1],
                           xy + log_odds_y(j + 1) + By[i][j + 1]);
      ////if(debug>1) fprintf(stdout, "%d %d: %d %f   %d %f   %d %f\n",
  ...
  ////BE = mm * Bm[1][1] * log_odds(1,1) + xy * Bx[1][0] * log_odds_x(1) + xy *
  ///By[0][1] * log_odds_y(1); /fprintf(stdout, "BE: %d %f Bm 0 0: %d %f\n",
  ///BE.INT, BE.DEC, Bm[0][0].INT, Bm[0][0].DEC);
  // BE = LOG_ADD(mm + Bm[1][1] + log_odds(1,1) , xy + Bx[1][0] + log_odds_x(1)
  // , xy + By[0][1] + log_odds_y(1) );
  if (debug > 1)
    BE = LOG_ADD(LOG(1 - 2 * theta) + Bm[1][1] + log_odds(1, 1),
                 LOG(theta) + Bx[1][0] + log_odds_x(1),
                 LOG(theta) + By[0][1] + log_odds_y(1));
...
void hmm_profpair::baumwelch() {
  ...
  */
  ...
  for (i = 1; i < lenx; i++) {
    for (j = 1; j < leny; j++) {
      ...
      */
      ...
      // cout << i << "\t" << j << ":\t" << gammaMM[i][j] << "\t" <<
      // LOG_ADD(mmmm[i][j], mmxx[i][j], mmyy[i][j]) << endl;
    }
  }
  // cout << "Here" << endl;
  ...
  // for(i=1;i<lenx;i++) { for(j=1;j<leny;j++) { cout << i << "\t" << j << ":\t"
  // << LOG_ADD(Fm[i][j]+Bm[i][j], Fx[i][j]+Bx[i][j], Fy[i][j]+By[i][j]) << "\t"
  // << BE << endl; } }

  cout << "CountEPx and CountEP1x: " << exp(countEPx) << "\t" << exp(countEP1x)
       << endl;
  cout << "CountEPy and CountEP1y: " << exp(countEPy) << "\t" << exp(countEP1y)
       << endl;
  cout << "CountDX, CountDy and CountD2m: " << exp(countDx) << "\t"
       << exp(countDy) << "\t" << exp(countD2m) << endl;
