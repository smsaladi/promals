...
inline ScoreType LOG(ScoreType x) { return log(x); }
...
inline float LOOKUP(float x) {
  ...
  // return ((-0.00653779113685f * x + 0.09537236626558f) * x +
  // 0.55317574459331f) * x + 0.68672959851568f;
  if (x <= 1.00f)
  ...
  // return (((0.00089738532761f * x - 0.01859488697982f) * x +
  // 0.14415772028626f) * x + 0.49515490689159f) * x + 0.69311928966454f;
}
...
inline float LOOKUP_SLOW(float x) { return log(exp(x) + 1); }
...
inline void LOG_PLUS_EQUALS(float &x, float y) {
  if (x < y)
    x = (x == LOG_ZERO || y - x >= LOG_UNDERFLOW_THRESHOLD) ? y
                                                            : LOOKUP(y - x) + x;
  else
    x = (y == LOG_ZERO || x - y >= LOG_UNDERFLOW_THRESHOLD) ? x
                                                            : LOOKUP(x - y) + y;
}
...
inline float LOG_ADD(float x1, float x2, float x3, float x4, float x5, float x6,
                     float x7) {
  return LOG_ADD(
      x1, LOG_ADD(x2, LOG_ADD(x3, LOG_ADD(x4, LOG_ADD(x5, LOG_ADD(x6, x7))))));
}
